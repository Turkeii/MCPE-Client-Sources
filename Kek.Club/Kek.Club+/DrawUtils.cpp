...
void DrawUtils::drawText(const vec2_t& pos, std::string* textStr, const MC_Color& color, float textSize, float alpha, Fonts font, bool shaddow) {
	TextHolder text(*textStr);
	C_Font* fontPtr = getFont(font);
	...
	if (hudModule->doShaddow)
		shaddow = true;

	renderCtx->drawText(fontPtr, posF, &text, color.arr, alpha, 0, &TextMeasureData(&textSize, shaddow), &caretMeasureData);
}
