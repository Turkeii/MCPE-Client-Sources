...
void ConfigManager::loadConfig(std::string name, bool create) {
	...
	if (configExists || create) {
		...
		if (g_Data.getLocalPlayer() != nullptr) {
			...
			g_Data.getGuiData()->displayClientMessageF("<%sKek.Club+%s> %sSuccessfully %s config %s%s%s!", GREEN, WHITE, GRAY, !configExists ? "created" : "loaded", GRAY, name.c_str(), GREEN);
			if (!helpedUser && name != "KekDefault") {
				helpedUser = true;
				g_Data.getGuiData()->displayClientMessageF("<%sKek.Club+%s> %sEnter \"%s%cconfig load default%s\" to load your old config!", GREEN, WHITE, GRAY, GRAY, cmdMgr->prefix, YELLOW);
			}
		}
	} else {
		if (g_Data.getLocalPlayer() != nullptr) 
			g_Data.getGuiData()->displayClientMessageF("<%sKek.Club+%s> %sCould not load config %s%s%s!", GREEN, WHITE, RED, GRAY, name.c_str(), RED);
	}
