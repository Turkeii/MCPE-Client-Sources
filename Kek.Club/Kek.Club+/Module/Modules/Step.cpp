...
bool Arora = false;
Step::Step() : IModule(0, Category::MOVEMENT, "Move up / down block big") {
	registerFloatSetting("height", &height, height, 1.f, 10.f);
	registerBoolSetting("Arora", &Arora, Arora);
}
...
void Step::onMove(C_MoveInputHandler* input) {
	if (Arora) {
		...
		if (!pressed)
			return;
		movevec2_td = movevec2_td.normalized();
		...
		for (int x = -1; x <= 1; x++) {
			for (int z = -1; z <= 1; z++) {
				...
				if (movevec2_td.dot(vec2_t((float)x, (float)z).normalized()) < 0.6f)
					continue;
		...
		if ((lowerObstructed || upperObstructed)) {
			...
			float getOver = lowerObstructed ? lowerObsVec.upper.y : upperObsVec.upper.y;
			{
				...
				const int numIterations = (int)ceil(std::max(5.f, -std::get<0>(distanceError(900.f, 0)) + 2));
...
void Step::onDisable() {
	if (g_Data.getLocalPlayer() != nullptr && !Arora)
