...
bool Scaffold::isUsefulBlock(C_ItemStack* itemStack) {
	...
	std::vector<std::string> uselessNames = {"cake", "ladder", "lever", "loom", "scaffolding", "web", "sand", "gravel", "dragon_egg", "anvil"};
...
void Scaffold::onTick(C_GameMode* gm) {
	...
	if (staircaseMode) {
		Utils::nopBytes((unsigned char*)NoSneakAddress, 2);
	}
...
void Scaffold::onDisable() {
	if (autoselect) {
	 ...
	 if (staircaseMode)
		Utils::patchBytes((unsigned char*)((uintptr_t)NoSneakAddress), (unsigned char*)"\x75\x0A", 2);
