...
SettingEntry* IModule::registerEnumSettingGroup(std::string name, SettingEnum* ptr, int defaultValue) {
	...
	size_t numToAdd = ptr->Entrys.size();
...
#endif
		if (Logmsg->isEnabled()) {
				g_Data.getClientInstance()->getGuiData()->displayClientMessageF("<%sKek.Club+%s> %s%s %s%s%s", GREEN, WHITE, GRAY, enabled ? "Enabled" : "Disabled",BOLD, WHITE,this->getModuleName());
