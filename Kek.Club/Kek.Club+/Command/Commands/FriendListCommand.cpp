...
bool FriendListCommand::execute(std::vector<std::string>* args) {
	...
	if (playerName.size() <= 1) {
		clientMessageF("<%sKek.Club+%s> %sCouldn't find player: %s!", GREEN, WHITE, RED, searchedName.c_str());
		return true;
	}
	if (subcommand == "add") {
		FriendList::addPlayerToList(playerName);
		clientMessageF("<%sKek.Club+%s> %s%s is now your friend!", GREEN, WHITE, GREEN, playerName.c_str());
		return true;

	} else if (subcommand == "remove") {
		if (FriendList::removePlayer(searchedName)) {
			clientMessageF("[%sKek.Club+%s] %s%s has been removed from your friendlist!", GREEN, WHITE, GREEN, searchedName.c_str());
			return true;
		} else {
			clientMessageF("<%sKek.Club+%s> %s%s was not in your friendlist!", GREEN, WHITE, GREEN, searchedName.c_str());
			return true;
		}
	}
	return true;
