...
bool PlayerTeleportCommand::execute(std::vector<std::string>* args) {
	...
	if (pos.iszero()) {
		clientMessageF("<%Kek.Club+%s> %sCouldn't find player: %s!", GREEN, WHITE, RED, nameOfPlayer.c_str());
		return true;
	}
	g_Data.getLocalPlayer()->setPos(pos);
	clientMessageF("<%Kek.Club+%s> %sTeleported to %s", GREEN, WHITE, GREEN, playerName.c_str());
	return true;
}
