...
struct MC_Color {
	...
	MC_Color(const bool idk, const float r, const float g, const float b, const float a = 255) {
		this->idk = idk;
		this->r = r / 255.0f;
		this->g = g / 255.0f;
		this->b = b / 255.0f;
...
public:
	...
	static void drawText(const vec2_t& pos, std::string* text, const MC_Color& color, float textSize = 1, float alpha = 1, Fonts font = Fonts::SMOOTH, bool shaddow = false);
