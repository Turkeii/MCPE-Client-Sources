...
void GameData::addChestToList(C_ChestBlockActor* chest) {
	if (chest == nullptr)// || !chest->isMainSubchest()) //chestBlock actor is le fucked D:
		return;
...
void GameData::sendPacketToInjector(BadManDataPacket horionDataPack) {
	...
	badmanToInjectorQueue.push(horionDataPack);
}
void GameData::callInjectorResponseCallback(int id, std::shared_ptr<BadManDataPacket> packet) {
	if (this->injectorToBadManResponseCallbacks.find(id) == this->injectorToBadManResponseCallbacks.end()) {
		logF("No response callback for request with id=%i!", id);
		return;
	}
	this->injectorToBadManResponseCallbacks[id](packet);
	this->injectorToBadManResponseCallbacks.erase(id);
}
