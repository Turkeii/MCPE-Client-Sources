...
#include "Module/Modules/AltTheme.h"
...
C_Font* DrawUtils::getFont(Fonts font) {
	...
	static auto TestyWesty = moduleMgr->getModule<Theme>();

	/*if (true)
		return g_Data.getClientInstance()->minecraftGame->getOldFont();*/

	if (TestyWesty->TheRealFonts.selected == 1)
...
void DrawUtils::drawNameTags(C_Entity* ent, float textSize, bool drawHealth, bool useUnicodeFont) {
	...
	if (refdef->OWorldToScreen(origin, ent->eyePos0.add(0, 0.5f, 0), textPos, fov, screenSize)) {
		...
		fillRectangle(rectPos, MC_Color(Background::rFloat, Background::gFloat, Background::bFloat), nameTagMod->opacity);
		fillRectangle(subRectPos, MC_Color(Background::rFloat + 0.3f, Background::gFloat + 0.3f, Background::bFloat + 0.3f), nameTagMod->opacity);
		drawText(textPos, &text, MC_Color(((float)nameTagMod->customR / (float)255), ((float)nameTagMod->customG / (float)255), ((float)nameTagMod->customB / (float)255), (float)fmax(0.1f, (float)fmin(1.f, 15 / (ent->damageTime + 1)))), textSize);
		if (FriendList::findPlayer(ent->getNameTag()->getText()) && !moduleMgr->getModule<NoFriends>()->isEnabled()) {
		drawText(textPos, &text, MC_Color(0.f, 1.f, 0.f), (float)fmax(0.1f, (float)fmin(1.f, 15 / (ent->damageTime + 1))), textSize);
		}
...
void DrawUtils::drawKeystroke(char key, vec2_t pos) {
	...
	pos.x += keyMod->keystrokesX;
	pos.y -= keyMod->keystrokesY;
	pos.x -= 9.f;
	pos.y += 9.f;
	vec4_t rectPos(
		pos.x,
	...
	if (ThemeMod->rainbow) {
		...
		fillRectangle(rectPos, GameData::isKeyDown(key) ? MC_Color(rcolors) : MC_Color(Background::rFloat, Background::gFloat, Background::bFloat), 1.f);
		if (ThemeMod->outline) {
			drawRectangle(rectPos, GameData::isKeyDown(key) ? MC_Color(Background::rFloat, Background::gFloat, Background::bFloat) : MC_Color(rcolors), 1.f);
		}
	} else {
		fillRectangle(rectPos, GameData::isKeyDown(key) ? MC_Color(Theme::rFloat, Theme::gFloat, Theme::bFloat) : MC_Color(Background::rFloat, Background::gFloat, Background::bFloat), 1.f);
		if (ThemeMod->outline) {
			drawRectangle(rectPos, GameData::isKeyDown(key) ? MC_Color(Background::rFloat, Background::gFloat, Background::bFloat) : MC_Color(Theme::rFloat, Theme::gFloat, Theme::bFloat), 1.f);
		}
	}
	drawText(textPos, &keyString, MC_Color(AltTheme::rFloat, AltTheme::gFloat, AltTheme::bFloat), 1.f, 1.f);
}
