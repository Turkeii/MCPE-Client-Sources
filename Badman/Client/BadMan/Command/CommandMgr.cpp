...
void CommandMgr::initCommands() {
	...
	commandList.push_back(new IDinfoCommand());
	...
	commandList.push_back(new DemiGodPot());
	commandList.push_back(new SummonMob());
...
void CommandMgr::execute(char* message) {
	if (message != nullptr) {
		...
		g_Data.getClientInstance()->getGuiData()->displayClientMessageF("[%sBadMan%s] %sCommand '%s' could not be found!", GOLD, WHITE, RED, cmd.c_str());
	}
}
