...
NbtCommand::NbtCommand() : IMCCommand("nbt", "read, save load & write NBT tags to/from your clipboard (You have to point at an entity/block entity)", "<read/save/load/write>") {
...
bool NbtCommand::execute(std::vector<std::string>* args) {
	...
	C_BlockActor* blockActor = g_Data.getLocalPlayer()->region->getBlockEntity(pointing->block);
	...
	if (args->at(1) == "read" || args->at(1) == "save") {
		...
		if (args->at(1) == "save" && item != nullptr) {
			...
			if (pointing->entityPtr != 0) {
				...
				pointing->entityPtr->save(tag.get());
				tag->write(build);
