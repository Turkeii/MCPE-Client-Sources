...
void TabGui::renderLevel() {
	...
	for (auto it = labelList.begin(); it != labelList.end(); ++it, i++) {
		...
		MC_Color color = MC_Color(AltTheme::rFloat, AltTheme::gFloat, AltTheme::bFloat);
		...
		if (selected[renderedLevel].selectedItemId == i && level >= renderedLevel) {  // We are selected
			if (renderedLevel == level) {                                             // Are we actually in the menu we are drawing right now?
				...
				static auto OpacityBG = moduleMgr->getModule<HudModule>();
		...
		if (ThemeMod->rainbow) {
			...
			Utils::ColorConvertHSVtoRGB(rcolors[0], rcolors[1], rcolors[2], rcolors[0], rcolors[1], rcolors[2]);
			DrawUtils::drawText(vec2_t(xOffset + 1.5f, yOffset + 0.5f), &tempLabel, label.enabled ? MC_Color(rcolors) : color, textSize);
		} else {
		DrawUtils::drawText(vec2_t(xOffset + 1.5f, yOffset + 0.5f), &tempLabel, label.enabled ? MC_Color(Theme::rFloat, Theme::gFloat, Theme::bFloat) : color, textSize);
		}
		yOffset += textHeight;
	}
	...
	// Draw selected item
	{
		...
		static auto OpacityBG = moduleMgr->getModule<HudModule>();
		DrawUtils::fillRectangle(selectedPos, MC_Color(Background::rFloat, Background::gFloat, Background::bFloat), OpacityBG->opacity);
		static auto ThemeMod = moduleMgr->getModule<Theme>();
		static float rcolors[4];
		if (ThemeMod->rainbow) {
			if (rcolors[3] < 1) {
			...
			Utils::ColorConvertHSVtoRGB(rcolors[0], rcolors[1], rcolors[2], rcolors[0], rcolors[1], rcolors[2]);
			if (ThemeMod->outline) {
				DrawUtils::drawRectangle(selectedPos, MC_Color(rcolors), 1.f);
			}
		} else {
			if (ThemeMod->outline) {
				DrawUtils::drawRectangle(selectedPos, MC_Color(Theme::rFloat, Theme::gFloat, Theme::bFloat), 1.f);
			}
		}
	}
...
void TabGui::render() {
	static auto Ylength = moduleMgr->getModule<HudModule>();
	static auto xlength = moduleMgr->getModule<HudModule>();
	if (!moduleMgr->isInitialized())
		return;
	if (!GameData::canUseMoveKeys())
		level = -1;
	renderedLevel = 0;
	yOffset = Ylength->Yheight;
	xOffset = xlength->Xheight;
