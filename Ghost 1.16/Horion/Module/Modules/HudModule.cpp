...
HudModule::HudModule() : IModule(0, Category::VISUAL, "Displays ArrayList/TabGUI") {
	...
	registerBoolSetting("Hello!", &this->hello, this->hello);
}
	...
	{  // Keystrokes
		if (!(g_Data.getLocalPlayer() == nullptr || !this->keystrokes || !GameData::canUseMoveKeys())) {
			...
			DrawUtils::drawKeystroke(*input->spaceBarKey, vec2_t(10.f, windowSize.y - 30));
				...
				{  // Hello thing
				if (!(g_Data.getLocalPlayer() == nullptr || !this->hello || !GameData::canUseMoveKeys())) {
					std::string fpsText = "Hello!";
					vec4_t rectPos = vec4_t(2.5f, startY + 30.f * scale, len, startY + 100.f * scale);
					vec2_t textPos = vec2_t(rectPos.y + 200.5f, rectPos.x + 3.f);
