...
NameSpoofCommand::NameSpoofCommand() : IMCCommand("namespoof", "BROKEN", "<name/reset> <name>") {
	registerAlias("ns");
...
bool NameSpoofCommand::execute(std::vector<std::string>* args) {
	if (args->at(1) == "name" && args->size() > 2) {
		...
		clientMessageF("[%sGhost Client%s] %sSet fakename to %s%s%s, please reconnect!", GOLD, WHITE, GREEN, GRAY, name->getText(), GREEN);
		return true;
	} else if (args->at(1) == "reset") {
		g_Data.setFakeName(NULL);
		clientMessageF("[%sGhost Client%s] %sReset fakename!", GOLD, WHITE, GREEN);
		return true;
	}
	return false;
}
