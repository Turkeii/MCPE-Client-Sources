...
bool PlayerTeleportCommand::execute(std::vector<std::string>* args) {
	...
	if (pos.iszero()) {
		clientMessageF("[%sGhost Client%s] %sCouldn't find player: %s!", GOLD, WHITE, RED, nameOfPlayer.c_str());
		return true;
	}
	g_Data.getLocalPlayer()->setPos(pos);
	clientMessageF("[%sGhost Client%s] %sTeleported to %s", GOLD, WHITE, GREEN, playerName.c_str());
	return true;
}
