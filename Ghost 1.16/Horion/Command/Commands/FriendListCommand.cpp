...
bool FriendListCommand::execute(std::vector<std::string>* args) {
	...
	if (playerName.size() <= 1) {
		clientMessageF("[%sGhost Client%s] %sCouldn't find player: %s!", GOLD, WHITE, RED, searchedName.c_str());
		return true;
	}
	if (subcommand == "add") {
		FriendList::addPlayerToList(playerName);
		clientMessageF("[%sGhost Client%s] %s%s is now your friend!", GOLD, WHITE, GREEN, playerName.c_str());
		return true;

	} else if (subcommand == "remove") {
		if (FriendList::removePlayer(searchedName)) {
			clientMessageF("[%sGhost Client%s] %s%s has been removed from your friendlist!", GOLD, WHITE, GREEN, searchedName.c_str());
			return true;
		} else {
			clientMessageF("[%sGhost Client%s] %s%s was not in your friendlist!", GOLD, WHITE, GREEN, searchedName.c_str());
			return true;
		}
	}
	return true;
