...
static constexpr float textPadding = 1.1f;
...
static constexpr float backgroundAlpha = 0.2;
static const MC_Color selectedModuleColor = MC_Color(75, 75, 75);
static const MC_Color selectedSettingColor1 = MC_Color(2, 2, 2);
static const MC_Color selectedSettingColor2 = MC_Color(2, 2, 2);
static const MC_Color moduleColor = MC_Color(2, 2, 2);
static const MC_Color SettingColor1 = MC_Color(2, 2, 2);
static const MC_Color SettingColor2 = MC_Color(2, 2, 2);
...
void ClickGui::renderTooltip(std::string* text) {
	...
	DrawUtils::fillRectangle(rectPos, MC_Color(2, 2, 2), 1.0f);
	DrawUtils::drawRectangle(rectPos, MC_Color(255, 255, 255), 1.f, 0.5f);
	DrawUtils::drawText(textPos, text, MC_Color(2, 255, 255), textSize);
}
						...
						for (auto setting : *settings) {
							...
							case ValueType::BOOL_T: {
								...
								// Checkbox
								{
									...
									DrawUtils::drawRectangle(boxPos, MC_Color(75, 75, 75), isFocused ? 1 : 0.8f, 0.5f);

									if (setting->value->_bool) {
										DrawUtils::setColor(180, 180, 180, 1);
							...
							case ValueType::FLOAT_T: {
								...
								// Slider
								{
									...
									// Visuals & Logic
									{
										...
										{
											rect.z = rect.x + value;
											DrawUtils::fillRectangle(rect, MC_Color(75, 75, 75), (areWeFocused || setting->isDragging) ? 1.f : 0.8f);
										}
...
void ClickGui::render() {
	...
	renderCategory(Category::BLATANT);
...
void ClickGui::onLoadConfig(void* confVoid) {
	...
	if (conf->contains("ClickGui")) {
		...
		if (obj.is_null())
			return;
		for (int i = 0; i <= (int)Category::BLATANT /*last category*/; i++) {
