...
void ConfigManager::loadConfig(std::string name, bool create) {
	...
	if (configExists || create) {
		...
		if (configExists) {
			...
			currentConfigObj["from"] = "Ghost Client";
		}
		...
		if (g_Data.getLocalPlayer() != nullptr) {
			...
			g_Data.getGuiData()->displayClientMessageF("[%sGhost Client%s] %sSuccessfully %s config %s%s%s!", GOLD, WHITE, GREEN, !configExists ? "created" : "loaded", GRAY, name.c_str(), GREEN);
			if (!helpedUser && name != "GCPublic") {
				helpedUser = true;
				g_Data.getGuiData()->displayClientMessageF("[%sGhost Client%s] %sEnter \"%s%cconfig load default%s\" to load your old config!", GOLD, WHITE, YELLOW, WHITE, cmdMgr->prefix, YELLOW);
			}
		}
	} else {
		if (g_Data.getLocalPlayer() != nullptr) 
			g_Data.getGuiData()->displayClientMessageF("[%sGhost Client%s] %sCould not load config %s%s%s!", GOLD, WHITE, RED, GRAY, name.c_str(), RED);
	}
