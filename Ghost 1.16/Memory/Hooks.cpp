	...
	if (messageHolder->getTextLength() > 0) {
		...
		if (*message == cmdMgr->prefix) {
			...
			*(DWORD*)(_this + 0xA94) = *(DWORD*)(v17 + 0x20);
			...
			if (!helpedUser) {
				helpedUser = true;
				g_Data.getClientInstance()->getGuiData()->displayClientMessageF("%sYour Ghost Leaked prefix is: \"%s%c%s\"", RED, YELLOW, cmdMgr->prefix, RED);
...
__int64 Hooks::RenderText(__int64 a1, C_MinecraftUIRenderContext* renderCtx) {
...

	{
		...
		if (!g_Data.isInjectorConnectionActive() && !hasSentWarning) {
			...
			if (elapsedFlot > 1.5f && !hasSentWarning) {
				hasSentWarning = true;
				auto box = g_Data.addInfoBox("You just injected!", "You are now even more EPIC than before, CONGRATS");
				box->closeTimer = 10;
...

	{
		...
		if (strcmp(screenName.c_str(), "start_screen") == 0) {
			// Draw BIG epic Ghost Client watermark
			//no
			{
				std::string text = "discord.gg/killaura";
				...
				DrawUtils::fillRectangle(rectPos, MC_Color(00, 00, 00), 1.f);
				DrawUtils::drawRectangle(rectPos, rcolors, 2.f, 2.f);
				DrawUtils::drawText(textPos, &text, rcolors, 8.f, 8.f);
			}
...

			{
				...
				if (shouldRenderWatermark) {
					...
					constexpr float versionTextSize = 0.5f;
					static const float textHeight = (nameTextSize + versionTextSize * 1.0f /* We don't quite want the version string in its own line, just a bit below the name */) * DrawUtils::getFont(Fonts::SMOOTH)->getLineHeight();
					constexpr float borderPadding = 1;
					constexpr float margin = 5;

					static std::string name = "Ghost Leaked by rxversed";
#ifdef _DEBUG
					static std::string version = "discord.gg/killaura";
#elif defined _BETA
					...
					DrawUtils::drawRectangle(rect, MC_Color(2, 2, 2), 1.f, 2.f);
					DrawUtils::fillRectangle(rect, MC_Color(rcolors), 1.f);
					DrawUtils::drawText(vec2_t(rect.x + borderPadding, rect.y), &name, MC_Color(2, 2, 2), nameTextSize);
					DrawUtils::drawText(vec2_t(rect.x + borderPadding + nameLength, rect.w - 7), &version, MC_Color(75, 75, 75), versionTextSize);
				}
				...
				if (moduleMgr->isInitialized() && shouldRenderArrayList) {
					...
					for (std::set<IModuleContainer>::iterator it = modContainerList.begin(); it != modContainerList.end(); ++it) {
						...
						vec4_t underline = vec4_t(
							...
							windowSize.x,
							leftRect.w + 1.f);
						DrawUtils::fillRectangle(underline, rcolors, 1.f);
						c++;
						b++;
						if (b < 20)
						...
						if (!GameData::canUseMoveKeys() && rectPos.contains(&mousePos) && hudModule->clickToggle) {
							...
							if (leftMouseDown) {
								DrawUtils::fillRectangle(selectedRect, MC_Color(100, 100, 100), 0.8f);
								if (executeClick)
									it->backingModule->toggle();
							} else
								DrawUtils::fillRectangle(selectedRect, MC_Color(100, 100, 100), 0.3f);
						}
	...
	// Draw Info / Alert Boxes
	{
		...
		if (box) {
			...
			DrawUtils::fillRectangle(rectPos, MC_Color(1, 1, 1), box->fadeVal);
...
HRESULT Hooks::swapChain__present(IDXGISwapChain* chain, UINT syncInterval, UINT flags) {
 ...
 \
	...
	context->IASetVertexBuffers(0, 0, &vertexBuffer, &stride, &offset);
...
void Hooks::LocalPlayer__updateFromCamera(__int64 a1, C_Camera* camera) {
	...
	auto SilentLookMod = moduleMgr->getModule<SilentLook>();
	auto noHurtcamMod = moduleMgr->getModule<NoHurtcam>();

	if (SilentLookMod->redirectMouse) {
		SilentLookMod->cameraFacesFront = camera->facesPlayerFront;
		SilentLookMod->isThirdPerson = camera->renderPlayerModel;
		if (SilentLookMod->resetViewTick >= 0) {
			camera->setOrientationDeg(SilentLookMod->lastCameraAngle.x, SilentLookMod->lastCameraAngle.y, 0);
		} else {
			camera->getPlayerRotation(&SilentLookMod->lastCameraAngle);
		}
