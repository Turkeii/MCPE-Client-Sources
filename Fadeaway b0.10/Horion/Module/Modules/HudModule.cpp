...
void HudModule::onPostRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	{  // FPS
		if (!(g_Data.getLocalPlayer() == nullptr || !this->fps)) {
			...
			DrawUtils::drawText(textPos, &fpsText, MC_Color(0, 0, 0), scale);
	...
	{  // CPS
		if (!(g_Data.getLocalPlayer() == nullptr || !this->cps)) {
			...
			vec2_t textPos = vec2_t(rectPos.x + 1.5f, rectPos.y + 1.f);
			DrawUtils::fillRectangle(rectPos, MC_Color(108, 0, 205), 1.f);
			DrawUtils::drawText(textPos, &cpsText, MC_Color(0, 0, 0), scale);
	...
	{  // Coordinates
		if (!(g_Data.getLocalPlayer() == nullptr || !this->coordinates)) {
			...
			vec2_t textPos = vec2_t(rectPos.x + 1.5f, rectPos.y + 1.f);
			DrawUtils::fillRectangle(rectPos, MC_Color(108, 0, 205), 1.f);
			DrawUtils::drawText(textPos, &coordsX, MC_Color(0, 0, 0), scale);
			textPos.y += f;
			DrawUtils::drawText(textPos, &coordsY, MC_Color(0, 0, 0), scale);
			textPos.y += f;
			DrawUtils::drawText(textPos, &coordsZ, MC_Color(0, 0, 0), scale);
		}
	}
	{  // ArmorHUD
