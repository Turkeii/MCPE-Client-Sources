...
static std::vector<C_Entity*> targetNutz;
void findNutz(C_Entity* currentEntity, bool isRegularEntity) {
...
void EntitySpider::onMove(C_MoveInputHandler* input) {
	targetNutz.clear();
	g_Data.forEachEntity(findNutz);

	if (!targetNutz.empty()) {
		auto player = targetNutz[0];
		if (player == nullptr)
			return;
