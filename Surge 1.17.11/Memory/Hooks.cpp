	...
	// Vtables
	{
		...
		// LocalPlayer::vtable
		{
			...
			if (localPlayerVtable == 0x0 || sigOffset == 0x0)
				...
				g_Hooks.Actor_antifireHook = std::make_unique<FuncHook>(localPlayerVtable[31], Hooks::Actor_antifire);
			}
		}
...
void Hooks::ClientInstanceScreenModel_sendChatMessage(void* _this, TextHolder* text) {
	...
	if (text->getTextLength() > 0) {
		...
		if (*message == cmdMgr->prefix) {
			...
			if (!helpedUser) {
				helpedUser = true;
				g_Data.getClientInstance()->getGuiData()->displayClientMessageF("%sYour Surge leaked by rxversed prefix is: \"%s%c%s\"", RED, YELLOW, cmdMgr->prefix, RED);
...

	{
		...
		if (strcmp(screenName.c_str(), "start_screen") == 0) {
			...
			if (g_Data.allowWIPFeatures() && g_Data.isInjectorConnectionActive()) {
				if (HImGui.Button("Load Dick Into Body", vec2_t(wid.x * (0.765f - 0.5f), wid.y * 0.92f), true)) {
				...
				if (HImGui.Button("Custom Dick Length", vec2_t(wid.x * 0.765f, wid.y * 0.92f), true)) {
					HorionDataPacket packet;
				...
				if (HImGui.Button("Custom Dick Color", vec2_t(wid.x * 0.5f, wid.y * 0.92f), true)) {
					HorionDataPacket packet;
					packet.cmd = CMD_FILECHOOSER;
...

			{
				...
				if (shouldRenderWatermark) {
					...
					constexpr float versionTextSize = 1.4f;
					static const float textHeight = (nameTextSize + versionTextSize * 0.8 /* We don't quite want the version string in its own line, just a bit below the name */) * DrawUtils::getFont(Fonts::SMOOTH)->getLineHeight();
					constexpr float borderPadding = 1;
					constexpr float margin = 5;

					static std::string name = "Surge|Leaked by rxversed#0001";
#ifdef _DEBUG
...
void Hooks::Actor_lerpMotion(C_Entity* _this, vec3_t motVec) {
	...
	if (noKnockbackmod->isEnabled()) {
		...
		if (!networkSender) {
			networkSender = reinterpret_cast<void*>(9 + FindSignature("48 8B CB ?? 90 40 ?? 00 00 ?? 47 11 ?? 48 8B ??"));
		}
...
void Hooks::Actor_antifire(C_Entity* _this) {
	//static auto oFunc = g_Hooks.Actor_antifireHook->GetFastcall<void, C_Entity*>();
	//static auto antifireMod = moduleMgr->getModule<AntiVisualFire>();
	//if (antifireMod->isEnabled()) return oFunc(_this);
}
