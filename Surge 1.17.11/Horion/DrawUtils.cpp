...
void DrawUtils::drawTracer(C_Entity* ent) {
	...
	// Rainbow color updates
	{
		...
		disabledRcolors[0] = std::min(1.f, rcolors[0] * 0.4f + 0.1f);
		disabledRcolors[1] = std::min(1.f, rcolors[1] * 0.3f + 0.2f);
		disabledRcolors[2] = std::min(1.f, rcolors[2] * 0.5f + 0.3f);
		disabledRcolors[3] = 1;
	}
	int a = 0;
	int b = 0;
	...
	if (tracerMod->type.selected == 1) {
		...
		if (target != vec2_t(0, 0)) {
			if (tracerMod->RGB) {
				DrawUtils::setColor(disabledRcolors[0], disabledRcolors[1], disabledRcolors[2], 1);
			} else {
...
void DrawUtils::drawBoxScaffold(const vec3_t& lower, const vec3_t& upper, float lineWidth, bool outline) {
	vec3_t diff;
	diff.x = upper.x - lower.x;
	diff.y = upper.y - lower.y;
	diff.z = upper.z - lower.z;
...
void DrawUtils::drawNameTags(C_Entity* ent, float textSize, bool drawHealth, bool useUnicodeFont) {
	...
	if (refdef->OWorldToScreen(origin, ent->eyePos0.add(0, 0.5f, 0), textPos, fov, screenSize)) {
		...
		fillRectangle(rectPos, MC_Color(0, 0, 255), nametagsMod->opacity);
		if (nametagsMod->underline) {
			fillRectangle(subRectPos, MC_Color(0, 0, 0), 0.9f);
		}
		drawText(textPos, &text, MC_Color(255, 255, 255), textSize);

		static auto nameTagsMod = moduleMgr->getModule<NameTags>();

		if (nameTagsMod->displayArmor) { // is player, show armor
...
void DrawUtils::drawKeystroke(char key, const vec2_t& pos) {
	...
	currColor[0] += 10.f / a * c;
	Utils::ColorConvertHSVtoRGB(currColor[0], currColor[0], currColor[1], currColor[0], currColor[0], currColor[1]);
	static auto RGBMod = moduleMgr->getModule<HudModule>();
	static auto opacitymod = moduleMgr->getModule<HudModule>();
	...
	if (key == *input->forwardKey) {
		...
		if (RGBMod->rgbtext) {
			drawText(textPos, &keyString, MC_Color(disabledRcolors), 1.f, 1.f);
		} else {
	...
	if (key == *input->spaceBarKey) {
		...
		fillRectangle(rectPos, GameData::isKeyDown(key) ? MC_Color(100, 100, 100) : MC_Color(0, 0, 0), opacitymod->opacity);
		if (RGBMod->rgbtext) {
			drawText(textPos, &keyString, MC_Color(disabledRcolors), 1.f, 1.f);
		} else {
			drawText(textPos, &keyString, MC_Color(0, 0, 255), 1.f, 1.f);
...
void DrawUtils::drawLeftMouseKeystroke(vec2_t pos) {
	...
	vec4_t rectPos(
		...
		pos.x + 31.f,
		pos.y + 22.f);
	fillRectangle(rectPos, GameData::GameData::isLeftClickDown() ? MC_Color(100, 100, 100) : MC_Color(0, 0, 0), opacitymod->opacity);
	vec2_t textPos(
		(rectPos.x + (rectPos.z - rectPos.x) / 2) - (DrawUtils::getTextWidth(&keyString) / 2.f),
		rectPos.y + 10.f - DrawUtils::getFont(Fonts::SMOOTH)->getLineHeight() / 2.f);
	if (RGBMod->rgbtext) {
...
void DrawUtils::drawRightMouseKeystroke(vec2_t pos) {
	...
	vec4_t rectPos(
		...
		pos.x + 31.f,
		pos.y + 22.f);
	fillRectangle(rectPos, GameData::GameData::isRightClickDown() ? MC_Color(100, 100, 100) : MC_Color(0, 0, 0), opacitymod->opacity);
	vec2_t textPos(
		(rectPos.x + (rectPos.z - rectPos.x) / 2) - (DrawUtils::getTextWidth(&keyString) / 2.f),
		rectPos.y + 10.f - DrawUtils::getFont(Fonts::SMOOTH)->getLineHeight() / 2.f);
	if (RGBMod->rgbtext) {
