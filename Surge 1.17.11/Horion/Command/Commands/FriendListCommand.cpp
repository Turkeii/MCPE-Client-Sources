...
bool FriendListCommand::execute(std::vector<std::string>* args) {
	...
	if (subcommand == "add") {
		...
		clientMessageF("[%sSurge%s] %s%s is now your friend!", GOLD, WHITE, GREEN, playerName.c_str());
		return true;

	} else if (subcommand == "remove") {
		if (FriendList::removePlayer(searchedName)) {
			clientMessageF("[%sSurge%s] %s%s has been removed from your friendlist!", GOLD, WHITE, GREEN, searchedName.c_str());
			return true;
		} else {
			clientMessageF("[%sSurge%s] %s%s was not in your friendlist!", GOLD, WHITE, GREEN, searchedName.c_str());
			return true;
		}
	}
	return true;
