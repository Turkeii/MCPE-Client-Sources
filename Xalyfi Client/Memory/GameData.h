...
private:
	...
	std::queue<BadManDataPacket> badmanToInjectorQueue;
	std::map<int, std::function<void(std::shared_ptr<BadManDataPacket>)>> injectorToBadManResponseCallbacks;
	int lastRequestId = 0;
...
public:
	...
	void sendPacketToInjector(BadManDataPacket badmanDataPack);
	inline int addInjectorResponseCallback(std::function<void(std::shared_ptr<BadManDataPacket>)> callback) {
		lastRequestId++;
		this->injectorToBadManResponseCallbacks[lastRequestId] = callback;
		return lastRequestId;
	}
	void callInjectorResponseCallback(int id, std::shared_ptr<BadManDataPacket> packet);
	...
	inline void setInjectorConnectionActive(bool isActive) {
		if (injectorConnectionActive && !isActive) {
			std::queue<BadManDataPacket> empty;
			badmanToInjectorQueue.swap(empty);
		}
		injectorConnectionActive = isActive;
	};
	inline bool isPacketToInjectorQueueEmpty() { return badmanToInjectorQueue.empty(); };
	inline BadManDataPacket getPacketToInjector() {
		if (isPacketToInjectorQueueEmpty())
			throw std::exception("Packet send queue is empty");
		BadManDataPacket pk = badmanToInjectorQueue.front();
		badmanToInjectorQueue.pop();
		return pk;
	};
