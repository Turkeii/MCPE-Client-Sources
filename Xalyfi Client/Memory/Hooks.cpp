...
#include "../BadMan/Module/Modules/Spammer.h"
	...
	if (messageHolder->getTextLength() > 0) {
		...
		if (*message == cmdMgr->prefix) {
			...
			*(DWORD*)(_this + 0xA94) = *(DWORD*)(v17 + 0x20);
			...
			if (!helpedUser) {
				helpedUser = true;
				g_Data.getClientInstance()->getGuiData()->displayClientMessageF("%sYour Xalyfi prefix is: \"%s%c%s\"", RED, YELLOW, cmdMgr->prefix, RED);
		...
		if (strcmp(screenName.c_str(), "start_screen") == 0) {
			// Draw BIG epic BadMan watermark
			{
				...
				if (ThemeMod->rainbow) {
					...
					DrawUtils::drawRectangle(rectPos, MC_Color(0.f, 1.f, 1.f), 1.f);
				}
			...
			if (g_Data.allowWIPFeatures()) {
				if (HImGui.Button("Welcome Xalyfi", vec2_t(wid.x * (0.765f - 0.5f), wid.y * 0.92f), true)) {
				...
				if (HImGui.Button("Konnichiwa", vec2_t(wid.x * 0.765f, wid.y * 0.92f), true)) {
					BadManDataPacket packet;
...

			{
				...
				if (shouldRenderWatermark) {
...
#ifdef _DEBUG
					static std::string version = " 1.31";
#elif defined _BETA
					static std::string version = " 1.31";
#else
					static std::string version = " 1.33";
#endif
					...
					//DrawUtils::fillRectangle(rect, MC_Color(12, 12, 12), 0.15f);
					DrawUtils::drawText(vec2_t(rect.x + borderPadding, rect.y), &name, MC_Color(0.8f, 0.74f, 1.f), nameTextSize);
					DrawUtils::drawText(vec2_t(rect.x + borderPadding + nameLength, rect.w - 7), &version, MC_Color(0.8f, 0.74f, 1.f), versionTextSize);
				}
				...
				if (moduleMgr->isInitialized() && shouldRenderArrayList) {
					...
					static auto login = moduleMgr->getModule<Verify>();
					...
					for (std::set<IModuleContainer>::iterator it = modContainerList.begin(); it != modContainerList.end(); ++it) {
						...
						if (!rainbowArray) {
							if (arrayMod->underline) {
								DrawUtils::fillRectangle(underline, MC_Color(0.f, 0.7f, 0.f), 0.25f);
							}
							if (arrayMod->outline) {
								DrawUtils::fillRectangle(leftRect, MC_Color(1.f, 0.f, 1.f), 1.f);
							}
						} else {
							if (arrayMod->outline) {
							...
							if (arrayMod->underline) {
								DrawUtils::fillRectangle(underline, MC_Color(0.7f, 0.7f, 1.f), 1.f);
							}
						}
						...
						if (rainbowArray) {
							coolarray = 0;
							DrawUtils::drawText(textPos, &textStr, MC_Color(0.f, 1.f, 1.f), textSize);
...
__int64 Hooks::ChestScreenController_tick(C_ChestScreenController* a1) {
	...
	static auto chestStealerMod = moduleMgr->getModule<Search>();
...
void prepCoolBean() {
	if (g_Data.getClientInstance() && g_Data.getClientInstance()->minecraftGame->getServerEntries() && *reinterpret_cast<__int64*>(g_Data.getClientInstance()->minecraftGame->getServerEntries() + 0x50)) {
		...
		struct BeansEntry {
			...
			TextHolder serverName;
			char filler2[8];
			std::shared_ptr<ThirdPartyServer>* start;  //	
