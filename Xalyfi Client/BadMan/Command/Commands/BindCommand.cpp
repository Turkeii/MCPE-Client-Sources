...
bool BindCommand::execute(std::vector<std::string>* args) {
	...
	if (!modOpt.has_value()) {
		clientMessageF("%Xalyfi:sCould not find module with name: %s", RED, moduleName.c_str());
		return true;
	}
	...
	if (args->size() >= 3) {
		...
		if (key.size() > 1) {
			...
			if (key == "none") {
				mod->setKeybind(0x0);
				clientMessageF("%sXalyfi:Successfully unbound %s!", GREEN, mod->getModuleName());
				return true;
			}
			...
			for (int i = 0; i < 190; i++) {
				...
				if (strcmp(needle, haystackLowercase) == 0) {
					mod->setKeybind(i);
					clientMessageF("%sXalyfi:The keybind of %s is now '%s'", GREEN, mod->getModuleName(), haystack);
					delete[] haystackLowercase;
					return true;
				}
				delete[] haystackLowercase;
			}
			clientMessageF("%sXalyfi:Invalid key!", RED);
			return true;
		}
		...
		if (keyCode >= 0x30 && keyCode <= 0x5A) {
			auto modOpt = moduleMgr->getModuleByName(moduleName);
			if (!modOpt.has_value()) {
				clientMessageF("%sXalyfi:Could not find module with name: %s", RED, moduleName.c_str());
			} else {
				modOpt.value()->setKeybind(keyCode);
				clientMessageF("%sXalyfi:The Keybind of %s is now '%c'", GREEN, mod->getModuleName(), keyCode);
			}
		} else {
			clientMessageF("%sXalyfi:Invalid Key! Outside of ascii range: %X", RED, keyCode);
		}
		return true;
	} else {
