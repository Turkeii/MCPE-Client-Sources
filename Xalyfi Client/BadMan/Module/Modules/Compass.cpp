...
void Compass::onPreRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	for (int off = -range; off <= range; off++) {
		...
		for (auto it = result.first; it != result.second; it++) {
			...
			DrawUtils::drawText(pos, &pName, MC_Color(AltTheme::rFloat, AltTheme::gFloat, AltTheme::bFloat), tSize, majorOpacity);
		}

		if (off != 0 && minorOpacity > 0) {
			if ((oDeg % 15) != 0) {
				if ((oDeg % 5) == 0) {
					DrawUtils::fillRectangle(vec4_t(xOff - 0.25f, 16, xOff + 0.25f, 19), MC_Color(AltTheme::rFloat, AltTheme::gFloat, AltTheme::bFloat), minorOpacity);
				}
				continue;
			}

			// Bigger line with degree displayed
			DrawUtils::fillRectangle(vec4_t(xOff - 0.5f, 15, xOff + 0.5f, 20), MC_Color(AltTheme::rFloat, AltTheme::gFloat, AltTheme::bFloat), minorOpacity);
			drawCenteredText(vec2_t(xOff, 20.f), std::to_string(oDeg), 0.75f, minorOpacity);
		}
	}

	// Center line
	DrawUtils::fillRectangle(vec4_t(sCenter - 0.5f, 15, sCenter + 0.5f, 25), MC_Color(AltTheme::rFloat, AltTheme::gFloat, AltTheme::bFloat), opacity);
...
void Compass::drawCenteredText(vec2_t pos, std::string text, float size, float textOpacity) {
	...
	DrawUtils::drawText(pos, &text, MC_Color(AltTheme::rFloat, AltTheme::gFloat, AltTheme::bFloat), size, textOpacity);
}
