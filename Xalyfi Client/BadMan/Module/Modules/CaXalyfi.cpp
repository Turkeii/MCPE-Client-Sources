#include "CaXalyfi.h"
CaXAS::CaXAS() : IModule(0x0, Category::COMBAT, "CrystalAura Developed by KaeruClient & sogeda & SBreality & ShinXe") {
...
int K3SRC2 = 0;
int K3SRC3 = 0;
const char* CaXAS::getModuleName() {
	return ("CaXAS");
}
...
void findEntityK3R(C_Entity* currentEntity, bool isRegularEntity) {
	static auto CaXASMod = moduleMgr->getModule<CaXAS>();
...
void CaXAS::onTick(C_GameMode* gm) {
	...
	g_Data.forEachEntity([](C_Entity* ent, bool b) {
		...
		int range = moduleMgr->getModule<CaXAS>()->range;
...
void CaXAS::onDisable() {
	K3SRC = 0;
	hasPlacedA = false;
}
void CaXAS::onPreRender(C_MinecraftUIRenderContext* renderCtx) {
