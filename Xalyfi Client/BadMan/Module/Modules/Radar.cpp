...
void Radar::onPreRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	DrawUtils::fillRectangle(vec4_t(0, topPad - cent, (float)size, topPad + cent), MC_Color(Background::rFloat, Background::gFloat, Background::bFloat), bgOpacity);
	...
	if (ThemeMod->rainbow) {
		...
		if (ThemeMod->outline) {
			DrawUtils::drawRectangle(vec4_t(0, topPad - cent, (float)size, topPad + cent), MC_Color(rcolors), bgOpacity);
		}
	} else {
		if (ThemeMod->outline) {
			DrawUtils::drawRectangle(vec4_t(0, topPad - cent, (float)size, topPad + cent), MC_Color(Theme::rFloat, Theme::gFloat, Theme::bFloat), bgOpacity);
		}
	}
