...
Auto32k::Auto32k() : IModule(0, Category::PLAYER, "Enchants Item In Hand Every Tick credits to 12brendon24") {
	this->registerIntSetting("Enchantment lvl", &this->EnchantmentLevel, 32767, 0, 32767);
...
const char* Auto32k::getModuleName() {
	return ("Auto32k");
}

void Auto32k::onTick(C_GameMode* gm) {
	C_PlayerInventoryProxy* supplies = g_Data.getLocalPlayer()->getSupplies();
	C_Inventory* inv = g_Data.getLocalPlayer()->getSupplies()->inventory;
	C_ItemStack* InHand = inv->getItemStack(supplies->selectedHotbarSlot);
	if (InHand->item != NULL) {
		int enchantId = 0;
		int enchantLevel = 32767;
		bool isAuto = true;

		enchantLevel = EnchantmentLevel;
