...
#define TEXTCOLOR (MC_Color(TextCustomizer::rFloat, TextCustomizer::gFloat, TextCustomizer::bFloat))
...
void ClickGui::renderTooltip(std::string* text) { //tooltip = hidari shita no setsumei
	//DrawUtils::doChroma();
		...
		for (auto& item : items) {
			...
			if (!item.isSetting) {
				...
				if (allowRender) {
					...
					// Text
					DrawUtils::drawText(textPos, &textStr, mod->isEnabled() ? MC_Color(Theme::rFloat, Theme::gFloat, Theme::bFloat) : MC_Color(AltTheme::rFloat, AltTheme::gFloat, AltTheme::bFloat), textSize);
				...
				default: {
					...
					DrawUtils::drawText(textPos, &elTexto, MC_Color(0, 255, 255), textSize);
				} break;
				}
			}
			currentYOffset += textHeight + (textPadding * 2);
...
void ClickGui::render() {
	...
	if (categoryOrder.empty()) {
		...
		categoryOrder.push_back(Category::GUI);
		categoryOrder.push_back(Category::SERVER);
	}
