...
void ButtonInfo::draw(vec2_t mousePos, const char* label) {
	...
	DrawUtils::drawText(textPos, &str, MC_Color(AltTheme::rFloat, AltTheme::gFloat, AltTheme::bFloat));
	if (isInSelectableSurface(mousePos)) {  // Mouse hovering over us
		DrawUtils::fillRectangle(surface, MC_Color(Theme::rFloat, Theme::gFloat, Theme::bFloat), 1);
		this->canClickB = true;
	} else {
		DrawUtils::fillRectangle(surface, MC_Color(Background::rFloat, Background::gFloat, Background::bFloat), 1);
