...
static int astolfoColors(int yOffset, int yTotal) {
	float speed = 2900;
	...
	hue += 0.5F;
	//return Utils::ColorConvertHSVtoRGB(hue, 1, 1, hue, hue, hue);
	return false;
}

void DrawUtils::drawRoundRectangle(vec4_t pos, const MC_Color col, float alpha) {
...
int DrawUtils::getRainbow(float seconds, float saturation, float brightness, long index) {
	/*auto time = std::chrono::system_clock::now();
	// for time we need to get the current ms and idk how lol
	float hue = (((time - index) % (int)(seconds * 1000)) / (float)(seconds * 1000));
	float hue5 = (((time + index) % (int)(seconds * 1000)) / (float)(seconds * 1000));
	float hue6 = (((time - index) % (int)(4000)) / (float)(4000));
	
	float hue2 = sin(sqrt(hue5) * (float)PI);

	float hue3 = sin(hue5 * hue5 * (float)PI);

	int color = Utils::HSBtoRGB(-hue6, saturation, brightness);

	int color2 = Utils::HSBtoRGB(hue3 / 7, saturation, brightness);

	return color;*/
	return false; // so it builds
}
