...
void IModule::setEnabled(bool enabled) {
	if (this->enabled != enabled) {
		...
		if (player == nullptr || ClickGUI->isEnabled() || isFlashMode() || !notifs->isEnabled() || strcmp(screenName.c_str(), "start_screen") == 0)
			shouldShow = false;

		if (notifs->showToggle && shouldShow) {
			auto CheckEnabled = enabled ? " Enabled" : " Disabled";
			auto notification = g_Data.addNotification("Notification:",std::string(std::string(CheckEnabled) + " " + this->getRawModuleName()));
			notification->duration = 3.f;
		}

		if (enabled) {
			this->onEnable();
			//PlaySoundA((LPCSTR)("C:\\Users\\ggpac\\AppData\\Local\\Packages\\Microsoft.MinecraftUWP_8wekyb3d8bbwe\\RoamingState\\PacketClient\\Assets\\Sounds\\Enable2.wav"), NULL, SND_FILENAME | SND_ASYNC);
		}
		else {
			//PlaySoundA((LPCSTR)("C:\\Users\\ggpac\\AppData\\Local\\Packages\\Microsoft.MinecraftUWP_8wekyb3d8bbwe\\RoamingState\\PacketClient\\Assets\\Sounds\\Disable2.wav"), NULL, SND_FILENAME | SND_ASYNC);
			this->onDisable();
		}
