...
#include "Module/Modules/TextCustomizer.h"
...
void DrawUtils::doChroma() {
	static float disabledRcolors[4];
	static float currColor[4];
	{
		Utils::ApplyRainbow(rcolors);
		disabledRcolors[0] = std::min(2.f, rcolors[0] * 0.4f + 0.2f);
		disabledRcolors[1] = std::min(2.f, rcolors[1] * 0.4f + 0.2f);
		disabledRcolors[2] = std::min(2.f, rcolors[2] * 0.4f + 0.2f);
		disabledRcolors[3] = 1;
	}
}

C_BaseActorRenderContext* basedCtx = nullptr;
...
void DrawUtils::setCtx(C_MinecraftUIRenderContext* ctx, C_GuiData* gui) {
	...
	if (basedCtx != nullptr)
		delete basedCtx;
	basedCtx = new C_BaseActorRenderContext(screenCtx, g_Data.getClientInstance(), g_Data.getClientInstance()->minecraftGame);
}
...
template <typename T>
inline T readPtr(uintptr_t start, size_t* offsets, size_t offsetCount) {
#define retIfBad(a, b) \
	if (IsBadReadPtr(a, sizeof(b))) return nullptr
	uintptr_t ptr = start;
	for (auto i = 0; i < offsetCount; i++) {
		size_t off = offsets[i];
		retIfBad((void*)ptr, uintptr_t);
...
C_Font* DrawUtils::getFont(Fonts font) {
	static C_Font* mojangles = nullptr;
	if (mojangles == nullptr) {
		uintptr_t addr = (uintptr_t)GetModuleHandleA("Minecraft.Windows.exe");
		size_t offsets[] = {0xC8, 0xF8, 0x8, 0x18, 0x278, 0x10, 0x0};
		mojangles = readPtr<C_Font*>(addr + 0x3693A80, offsets, 7);

		//logF("mojangles at %llX", reinterpret_cast<uintptr_t>(mojangles));
	}

	if (moduleMgr->getModule<NotoSans>()->isEnabled() || mojangles == nullptr)  // Don't crash if the pointer is invalid
...
/*
void DrawUtils::drawText(vec2_t pos, std::string* textStr, MC_Color color, float textSize, float alpha, Fonts font) {
	if (!moduleMgr->getModule<NotoSans>()->isEnabled()) {  // new font
		...
		TextMeasureData textMeasure{};
		memset(&textMeasure, 0, sizeof(TextMeasureData));
		textMeasure.textSize = textSize - 0.19f;

		renderCtx->drawDebugText(posF, text, color.arr, 1, 0, &textMeasure, &caretMeasureData);
	} else {  //old font
...
void DrawUtils::drawNameTags(C_Entity* ent, float textSize, bool drawHealth, bool useUnicodeFont) {
	...
	std::string gamerText = std::to_string((int)ent->getHealth() /* Don't show the .0 */);
	text = Utils::sanitize(text);

	float textWidth = getTextWidth(&text, textSize);
	
	if (drawHealth) {
		textWidth += 3.f * textSize;
		textWidth += DrawUtils::getTextWidth(&gamerText, textSize);
	}
	...
	if (refdef->OWorldToScreen(origin, ent->eyePos0.add(0, 0.5f, 0), textPos, fov, screenSize)) {
		...
		fillRectangle(rectPos, MC_Color(20, 20, 20), 0.8f);
		...
		if (drawHealth) {
			vec2_t gamerPos(rectPos.z - 1.f - DrawUtils::getTextWidth(&gamerText, textSize), textPos.y);
			...
			DrawUtils::drawText(gamerPos, &gamerText, gamerColor, textSize);
		}
...
void DrawUtils::drawItem(C_ItemStack* item, vec2_t itemPos, float opacity, float scale, bool isEnchanted) {
	...
	__int64 scnCtx = reinterpret_cast<__int64*>(renderCtx)[2];
	auto* screenCtx = reinterpret_cast<C_ScreenContext*>(scnCtx);
	C_BaseActorRenderContext baseActorRenderCtx = *basedCtx;
	C_ItemRenderer* renderer = baseActorRenderCtx.renderer;

	C_ItemStack* newItem = nullptr;

	/*{
		// Step 1: Make an item from ItemRegistry
		void* ItemPtr = malloc(0x8);
		C_Item*** cStack = ItemRegistry::getItemFromId(ItemPtr, item->getItem()->itemId);
		if (!cStack || !*cStack || !**cStack) {
			goto RENDER;
		}
		
		newItem = new C_ItemStack(***cStack, 1, 0);
	}*/

RENDER:
	C_ItemStack* stack = newItem == nullptr ? item : newItem;

	renderer->renderGuiItemNew(&baseActorRenderCtx, newItem == nullptr ? item : newItem, g_Data.getClientInstance()->minecraftGame, itemPos.x, itemPos.y, opacity, scale, isEnchanted);

	/*vec4_t rect(
		itemPos.x,
		itemPos.y + (13.f * scale),
		itemPos.x + (15.f * scale),
		itemPos.y + (15.f * scale)
	);
	fillRectangle(rect, MC_Color(0, 0, 0), 1.f);

	{
		float maxDamage = *((unsigned __int16*)(newItem->getItem()) + 172);
		...
		if (stack->tag != nullptr){
			...
			std::string tag = ez.str();
			size_t dmgPos;
			if ((dmgPos = tag.find("Damage:")) != std::string::npos) {
				__int64 other;
				for (int i = dmgPos; i < tag.size(); i++) {
				...
				std::string num = tag.substr(dmgPos, other - dmgPos + 1);
				int converted = 0;
				try {
					converted = std::stoi(num);
				}
...
#define txtColor MC_Color(TextCustomizer::rFloat, TextCustomizer::gFloat, TextCustomizer::bFloat)
#define keystrkeColor MC_Color(Keystrokes::rFloat, Keystrokes::gFloat, Keystrokes::bFloat)
#define color MC_Color(ColorCustomizer::rFloat, ColorCustomizer::gFloat, ColorCustomizer::bFloat)
#define colorPressed MC_Color(ColorCustomizer::rFloat + 65, ColorCustomizer::gFloat + 65, ColorCustomizer::bFloat + 65)
...
void DrawUtils::drawKeystroke(char key, vec2_t pos) {
	doChroma();
	bool isSmooth = !moduleMgr->getModule<NotoSans>()->isEnabled();

	if (isSmooth) {
		//kinda lazy way to fix and a bit confusing but this secction is the new font and the other section is mojangles / old font
		...
		if (key == *input->forwardKey) {
			...
			if (Keystrokes::outline == true) {
				if (Keystrokes::chroma == true) {
					drawRectangle(rectPos, rcolors, 1.f, 1.f);
				} else {
					drawRectangle(rectPos, keystrkeColor, 1.f, 1.f);
				}
			}

			fillRectangle(rectPos, GameData::isKeyDown(key) ? FILLCOLORSMORT : FILLCOLOR, ColorCustomizer::opacity);
			if (Keystrokes::chroma == false) {
				drawText(textPos, &keyString, txtColor, 1.f, 1.f);
			} else {
		...
		if (key == *input->forwardKey) {
			vec4_t rectPos(
				...
				pos.y + 20.f);
			vec2_t textPos(
				(rectPos.x + (rectPos.z - rectPos.x) / 2) - (DrawUtils::getTextWidth(&keyString) / 2.f) + 0.7,  // fix the w key bug
				rectPos.y + 11.f - DrawUtils::getFont(Fonts::SMOOTH)->getLineHeight() / 2.f);
			if (Keystrokes::outline == true) {
				if (Keystrokes::chroma == true) {
					drawRectangle(rectPos, rcolors, 1.f, 1.f);
				} else {
			...
			if (key == *input->spaceBarKey) {
				...
				fillRectangle(rectPos, GameData::isKeyDown(key) ? colorPressed : color, ColorCustomizer::opacity);
				if (Keystrokes::chroma == false) {
					drawText(textPos, &keyString, txtColor, 1.f, 1.f);
				} else {
					drawText(textPos, &keyString, MC_Color(rcolors), 1.f, 1.f);
				...
				vec4_t rectPos(
					...
					pos.y + 20.f);
				vec2_t textPos(
					(rectPos.x + (rectPos.z - rectPos.x) / 2) - (DrawUtils::getTextWidth(&keyString) / 2.f) + 0.7f,
					rectPos.y + 11.f - DrawUtils::getFont(Fonts::SMOOTH)->getLineHeight() / 2.f);
				if (Keystrokes::outline == true) {
					if (Keystrokes::chroma == true) {
						drawRectangle(rectPos, rcolors, 1.f, 1.f);
...
void DrawUtils::drawLeftMouseKeystroke(vec2_t pos) {
	...
	keyString = "LMB";
	std::string cpsText = std::to_string(g_Data.getLeftCPS()) + " CPS";
	vec4_t rectPos(
		pos.x,
		pos.y + 2,
		pos.x + 31.f,
		pos.y + 22.f);
	fillRectangle(rectPos, GameData::GameData::isLeftClickDown() ? MC_Color(FILLCOLORSMORT) : MC_Color(FILLCOLOR), ColorCustomizer::opacity);
	if (!moduleMgr->getModule<NotoSans>()->isEnabled()) {
		if (Keystrokes::cps == true) {
			vec2_t textPos(
				(rectPos.x + (rectPos.z - rectPos.x) / 2) - (DrawUtils::getTextWidth(&keyString) / 2.f),
				rectPos.y + 6.f - DrawUtils::getFont(Fonts::SMOOTH)->getLineHeight() / 2.f);
			...
			vec2_t textPos1(
				(rectPos.x + (rectPos.z - rectPos.x) / 2) - (DrawUtils::getTextWidth(&cpsText) / 2.f) + 1.8f,
				rectPos.y + 15.f - DrawUtils::getFont(Fonts::SMOOTH)->getLineHeight() / 2.f);
			if (Keystrokes::chroma == true) {
				DrawUtils::drawText(textPos1, &cpsText, MC_Color(rcolors), 0.8f);
			} else {
				DrawUtils::drawText(textPos1, &cpsText, txtColor, 0.8f);
			}
			if (Keystrokes::outline == true) {
				if (Keystrokes::chroma == true) {
					drawRectangle(rectPos, rcolors, 1.f, 1.f);
		...
		} else if (Keystrokes::cps == false) {
			vec2_t textPos(
				(rectPos.x + (rectPos.z - rectPos.x) / 2) - (DrawUtils::getTextWidth(&keyString) / 2.f),
				rectPos.y + 9.f - DrawUtils::getFont(Fonts::SMOOTH)->getLineHeight() / 2.f);
			if (Keystrokes::chroma == true) {
				drawText(textPos, &keyString, MC_Color(rcolors), 1.f, 1.f);
			} else {
				drawText(textPos, &keyString, txtColor, 1.f, 1.f);
		...
		if (Keystrokes::cps == true) {
			...
			vec2_t textPos1(
				(rectPos.x + (rectPos.z - rectPos.x) / 2) - (DrawUtils::getTextWidth(&cpsText) / 2.f) + 3,
				rectPos.y + 17.f - DrawUtils::getFont(Fonts::SMOOTH)->getLineHeight() / 2.f);
			if (Keystrokes::chroma == true) {
				DrawUtils::drawText(textPos1, &cpsText, MC_Color(rcolors), 0.8f);
			} else {
				DrawUtils::drawText(textPos1, &cpsText, txtColor, 0.8f);
...
void DrawUtils::drawRightMouseKeystroke(vec2_t pos) {
	...
	std::string keyString;
	keyString = "RMB";
	std::string cpsText = std::to_string(g_Data.getRightCPS()) + " CPS";
	vec4_t rectPos(
		pos.x,
		pos.y + 2,
		pos.x + 31.f,
		pos.y + 22.f);
	fillRectangle(rectPos, GameData::GameData::isRightClickDown() ? FILLCOLORSMORT : FILLCOLOR, ColorCustomizer::opacity);
	if (!moduleMgr->getModule<NotoSans>()->isEnabled()) {
		if (Keystrokes::cps == true) {
			vec2_t textPos(
				(rectPos.x + (rectPos.z - rectPos.x) / 2) - (DrawUtils::getTextWidth(&keyString) / 2.f),
