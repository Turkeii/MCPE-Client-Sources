...
DWORD WINAPI injectorConnectionThread(LPVOID lpParam) {
	...
	auto** DaDripToInjectorPtr = reinterpret_cast<MemoryBoi**>(magicArray + sizeof(magicValues));
	auto** injectorToDaDripPtr = reinterpret_cast<MemoryBoi**>(magicArray + sizeof(magicValues) + sizeof(uintptr_t));

	*DaDripToInjectorPtr = new MemoryBoi();
	MemoryBoi* DaDripToInjector = *DaDripToInjectorPtr;
	DaDripToInjector->isPresent = true;
	DaDripToInjector->protocolVersion = 1;

	*injectorToDaDripPtr = new MemoryBoi();
	MemoryBoi* injectorToDaDrip = *injectorToDaDripPtr;

	magicArray[0] = 0x48;  //Only find this allocated one, not the one in the thread stack

	logF("DaDripToInjectorPtr at %llX", DaDripToInjector);
	logF("injectorToDaDripPtr at %llX", injectorToDaDrip);
	...
	while (isRunning) {
		...
		bool isConnected = DaDripToInjector->isPresent && injectorToDaDrip->isPresent && DaDripToInjector->protocolVersion >= injectorToDaDrip->protocolVersion;

		if (isConnected && !injectorToDaDrip->isUnread) {
			...
			if (realElapsed > 4.f) {
				...
				injectorToDaDrip->isPresent = false;
				QueryPerformanceCounter(&timeSinceLastMessage);
			}
		}
		...
		if (isConnected) {
			...
			if (injectorToDaDrip->isUnread) {  // They sent us a message
				QueryPerformanceCounter(&timeSinceLastMessage);
				switch (injectorToDaDrip->cmd) {
				case CMD_INIT: {
					logF("Got CMD_INIT from injector");
					loggedIn = true;
					int flags = injectorToDaDrip->params[0];
					if (flags & (1 << 0) && injectorToDaDrip->dataSize > 0 && injectorToDaDrip->dataSize < sizeof(injectorToDaDrip->data)) {  // Has Json data
						injectorToDaDrip->data[sizeof(injectorToDaDrip->data) - 1] = '\0';
						json data = json::parse(reinterpret_cast<char*>(injectorToDaDrip->data));
				...
				case CMD_PING: {
					DaDripDataPacket pongPacket;
					pongPacket.cmd = CMD_PONG;
					pongPacket.params[0] = injectorToDaDrip->params[0];
				...
				case CMD_RESPONSE: {
					int id = injectorToDaDrip->params[1];

					auto pk = std::make_shared<DaDripDataPacket>();
					pk->cmd = (DATAPACKET_CMD)injectorToDaDrip->params[0];
					memcpy(pk->params, &injectorToDaDrip->params[2], sizeof(injectorToDaDrip->params) - sizeof(int) * 2);

					if (injectorToDaDrip->dataSize > 0 && injectorToDaDrip->dataSize < 3000) {
						pk->dataArraySize = injectorToDaDrip->dataSize;

						auto dataTemp = new unsigned char[injectorToDaDrip->dataSize + 2];
						memset(dataTemp + injectorToDaDrip->dataSize, 0, 2);  // If we don't zero the last 2 bytes, printing as unicode string won't work
						memcpy(dataTemp, injectorToDaDrip->data, injectorToDaDrip->dataSize);
				...
				case CMD_LOG: {
					if(injectorToDaDrip->dataSize > 4 && injectorToDaDrip->dataSize < 2999){
						injectorToDaDrip->data[injectorToDaDrip->dataSize] = 0; // null terminator

						char* command = reinterpret_cast<char*>(&injectorToDaDrip->data[3]);
				...
				injectorToDaDrip->isUnread = false;
			}
			...
			if (!DaDripToInjector->isUnread && !g_Data.isPacketToInjectorQueueEmpty()) {
				QueryPerformanceCounter(&timeSinceLastMessage);
				// They read the message, lets send the next one
				DaDripDataPacket nextDataPack = g_Data.getPacketToInjector();
				if (nextDataPack.dataArraySize >= 3000) {
					throw std::exception("DaDrip Data packet too big to send");
				}

				DaDripToInjector->cmd = nextDataPack.cmd;
				memcpy(DaDripToInjector->params, nextDataPack.params, sizeof(int) * 5);
				if (nextDataPack.dataArraySize > 0) {
					memcpy(DaDripToInjector->data, nextDataPack.data.get(), nextDataPack.dataArraySize);
				}

				DaDripToInjector->dataSize = nextDataPack.dataArraySize;

				DaDripToInjector->isUnread = true;
			}
		} else
			Sleep(30);
	}
	logF("Quitting connection thread");
	DaDripToInjector->isPresent = false;
	memset(magicValues, 0, sizeof(magicValues));
	memset(magicArray, 0, sizeof(magicValues) + sizeof(uintptr_t) * 2);
	Sleep(150);
	delete *DaDripToInjectorPtr;
	delete *injectorToDaDripPtr;
	delete[] magicArray;
...
DWORD WINAPI start(LPVOID lpParam) {
	...
	if (!g_Data.verifyXuid()) {
		logF("Invalid auth detected, shutting down...");
		GameData::terminate();
		return 1;
	}
