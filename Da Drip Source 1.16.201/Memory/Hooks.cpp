	...
	// Signatures
	{
		...
		static auto bobViewHookF = [](__int64 _this, glm::mat4& matrix, float lerpT){
			...
			if (testMod->isEnabled()) {
				...
				if (testMod->doRotate)
					matrix = glm::rotate<float>(matrix, glm::radians<float>(degrees), glm::vec3(testMod->xRotate, testMod->yRotate, testMod->zRotate));
			}
		...
		// Static function, no _this parameter
		void* isPaymentItem = (void*)FindSignature("48 89 5C 24 ?? 48 89 7C 24 ?? 55 48 8D AC 24 ?? ?? ?? ?? 48 81 EC ?? ?? ?? ?? 48 8B 05 ?? ?? ?? ?? 48 33 C4 48 89 85 ?? ?? ?? ?? 48 8B D9 80 79 ?? ?? 74 ?? E8 ?? ?? ?? ?? 80 7B ?? ??");
		g_Hooks.BeaconBlockActor_isPaymentItemHook = std::make_unique<FuncHook>(isPaymentItem, Hooks::BeaconBlockActor_isPaymentItem);
	...
	if (messageHolder->getTextLength() > 0) {
		...
		if (*message == cmdMgr->prefix) {
			...
			*(DWORD*)(_this + 0xA94) = *(DWORD*)(v17 + 0x20);
			...
			if (!helpedUser) {
				helpedUser = true;
				g_Data.getClientInstance()->getGuiData()->displayClientMessageF("%sYour DaDrip prefix is: \"%s%c%s\"", RED, YELLOW, cmdMgr->prefix, RED);
...
__int64 Hooks::RenderText(__int64 a1, C_MinecraftUIRenderContext* renderCtx) {
	...
	static auto textCustomizerMod = moduleMgr->getModule<TextCustomizer>();
		...
		if (strcmp(screenName.c_str(), "start_screen") == 0) {
...

			{
				...
				if (textCustomizerMod->chroma) {
					...
					DrawUtils::drawRectangle(rectPos, MC_Color(TextCustomizer::rFloat, TextCustomizer::gFloat, TextCustomizer::bFloat), 1.f);
				}
			...
			if (g_Data.allowWIPFeatures()) {
				if (HImGui.Button("Load Script Folder", vec2_t(wid.x * (0.765f - 0.5f), wid.y * 0.92f), true)) {
					...
					packet.params[0] = g_Data.addInjectorResponseCallback([](std::shared_ptr<DaDripDataPacket> pk) {
			...
			shouldRenderTabGui = hudModule->isEnabled();
...

			{
				...
				if (moduleMgr->isInitialized() && shouldRenderArrayList) {
					...
					rainbowArray = arrayListMod->rainbow && arrayListMod->isEnabled();
					{
					...
					for (std::set<IModuleContainer>::iterator it = modContainerList.begin(); it != modContainerList.end(); ++it) {
						...
						DrawUtils::fillRectangle(rectPos, MC_Color(0.f, 0.f, 0.f), 0.25f);
						...
						if (rainbowArray) {
							...
							DrawUtils::drawText(textPos, &textStr, MC_Color(ArrayList::rFloat, ArrayList::gFloat, ArrayList::bFloat), textSize);
						}
...
void Hooks::LoopbackPacketSender_sendToServer(C_LoopbackPacketSender* a, C_Packet* packet) {
	...
	static auto speedBypassMod = moduleMgr->getModule<SpeedBypass>();
	...
	if (chatMod->isEnabled()) {
		if (chatMod->greenText) {
			if (packet->isInstanceOf<C_TextPacket>()) {
				C_TextPacket* funi = reinterpret_cast<C_TextPacket*>(packet);
				std::stringstream build;
				build << "> ";
				build << funi->message.getText();

				funi->message.setText(build.str().c_str());
			}
		}
		if (chatMod->amongUsDrip) {
			if (packet->isInstanceOf<C_TextPacket>()) {
				C_TextPacket* funi = reinterpret_cast<C_TextPacket*>(packet);
				std::stringstream build;
				build << funi->message.getText();
	...
	if (freecamMod->isEnabled() || blinkMod->isEnabled() || speedBypassMod->isEnabled()) {
		if (packet->isInstanceOf<C_MovePlayerPacket>() || packet->isInstanceOf<PlayerAuthInputPacket>()) {
			if (blinkMod->isEnabled() || speedBypassMod->isEnabled()) {
				if (packet->isInstanceOf<C_MovePlayerPacket>()) {
					...
					blinkMod->getMovePlayerPacketHolder()->push_back(new C_MovePlayerPacket(*meme));
	...
	if (autoAnvilMod->isEnabled() && g_Data.getLocalPlayer() != nullptr && autoAnvilMod->sneak && packet->isInstanceOf<C_PlayerActionPacket>()) {
		auto* pp = reinterpret_cast<C_PlayerActionPacket*>(packet);
...
void Hooks::GameMode_startDestroyBlock(C_GameMode* _this, vec3_ti* a2, uint8_t face, void* a4, void* a5) {
	...
	if (instaBreakModule->isEnabled()) {
		if (instaBreakModule->dontBedrock && g_Data.getLocalPlayer()->region->getBlock(*a2)->toLegacy()->blockId != 7) {
			if (instaBreakModule->minBreak >= instaDelay) {
		...
		} else if (!instaBreakModule->dontBedrock) {
			if (instaBreakModule->minBreak >= instaDelay) {
				_this->destroyBlock(a2, face);
				instaDelay = 0;
				return;
...
void Hooks::ClickFunc(__int64 a1, char mouseButton, char isDown, __int16 mouseX, __int16 mouseY, __int16 relativeMovementX, __int16 relativeMovementY, char a8) {
	...
	if (clickGuiModule->isEnabled()) {
		...
		if (mouseButton != 0)  // if not a mouse move, Cancel it!
			return;
	}
...
void prepCoolBean() {
	if (g_Data.getClientInstance() && g_Data.getClientInstance()->minecraftGame->getServerEntries() && *reinterpret_cast<__int64*>(g_Data.getClientInstance()->minecraftGame->getServerEntries() + 0x50)) {
...

		{
			...
			t->serverName.setText("DaDrip Server");
			t->serverName2.setText("DaDrip Server");  // This is the one actually displayed
			t->domain.setText(".DaDripbeta.club");
			t->serverAddress.setText("mc.DaDripbeta.club");
...
bool Hooks::BeaconBlockActor_isPaymentItem(void* a1) {
	if (moduleMgr->getModule<TestModule>()->isEnabled())
		return true;

	auto oFunc = g_Hooks.BeaconBlockActor_isPaymentItemHook->GetFastcall<bool, void*>();
	return oFunc(a1);
}
