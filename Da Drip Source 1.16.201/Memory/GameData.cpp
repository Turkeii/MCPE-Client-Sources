...
void GameData::retrieveClientInstance() {
	...
	if (!g_Data.networkedData.dataSet) {
		g_Data.clientInstance = 0;
		return;
	}

	clientInstanceOffset = g_Data.networkedData.clientInstanceOffset;  // pointer scanned, can't find good signatures so it'll stay
...
void GameData::sendPacketToInjector(DaDripDataPacket DaDripDataPack) {
	if (!isInjectorConnectionActive())
		throw std::exception("DaDrip injector connection not active");
	if (DaDripDataPack.dataArraySize >= 3000) {
		logF("Tried to send data packet with array size: %i %llX", DaDripDataPack.dataArraySize, DaDripDataPack.data.get());
		throw std::exception("Data packet data too big");
	}

	DaDripToInjectorQueue.push(DaDripDataPack);
}
void GameData::callInjectorResponseCallback(int id, std::shared_ptr<DaDripDataPacket> packet) {
	if (this->injectorToDaDripResponseCallbacks.find(id) == this->injectorToDaDripResponseCallbacks.end()) {
		logF("No response callback for request with id=%i!", id);
		return;
	}
	this->injectorToDaDripResponseCallbacks[id](packet);
	this->injectorToDaDripResponseCallbacks.erase(id);
}
...
bool GameData::verifyXuid() {
	auto xuid = getPlayerXuid();

	char theFunny[64];
	// Bad auth is 12
	// Good auth is __
	sprintf_s(theFunny, "%llX", xuid);

	char url[128];
	sprintf_s(url, "%s", XorString("http://oldfagbeta.club/auth/ExileAuth.php"));

	http::Request request(url);

	const http::Response response = request.send("POST", std::string(theFunny), {"Content-Type: application/x-www-form-urlencoded"});

	char responseStr[64];
	memset((void*)&responseStr, 0x0, 63);  // Null terminator

	for (int i = 0; i < response.body.size() && i < 63; i++) {
		responseStr[i] = (char)response.body[i];
	}
	...
	std::string _json = str.substr(i);
	auto obj = nlohmann::json::parse(_json);

	if (obj.contains("succeeded"))
		if (!obj.at("succeeded").is_null())
			if (obj.at("succeeded").is_boolean())
				if (!obj.at("succeeded").get<bool>())
					return false;

	if (obj.contains("base")) {
		if (!obj.at("base").is_null()) {
			if (obj.at("base").is_number()) {
				g_Data.networkedData.clientInstanceOffset = obj.at("base").get<uintptr_t>();
