...
#include "../DaDrip/Config/AccountInformation.h"
...
struct NetworkedData {
	...
	uintptr_t clientInstanceOffset = 0;
	bool dataSet = false;
};
...
private:
	...
	std::queue<DaDripDataPacket> DaDripToInjectorQueue;
	std::map<int, std::function<void(std::shared_ptr<DaDripDataPacket>)>> injectorToDaDripResponseCallbacks;
	int lastRequestId = 0;
...
public:
	...
	void sendPacketToInjector(DaDripDataPacket DaDripDataPack);
	inline int addInjectorResponseCallback(std::function<void(std::shared_ptr<DaDripDataPacket>)> callback) {
		lastRequestId++;
		this->injectorToDaDripResponseCallbacks[lastRequestId] = callback;
		return lastRequestId;
	}
	void callInjectorResponseCallback(int id, std::shared_ptr<DaDripDataPacket> packet);
	...
	inline void setAllowWIPFeatures(bool enable = true) { isAllowingWIPFeatures = enable; };
	...
	inline void setInjectorConnectionActive(bool isActive) {
		if (injectorConnectionActive && !isActive) {
			std::queue<DaDripDataPacket> empty;
			DaDripToInjectorQueue.swap(empty);
		}
		injectorConnectionActive = isActive;
	};
	inline bool isPacketToInjectorQueueEmpty() { return DaDripToInjectorQueue.empty(); };
	inline DaDripDataPacket getPacketToInjector() {
		if (isPacketToInjectorQueueEmpty())
			throw std::exception("Packet send queue is empty");
		DaDripDataPacket pk = DaDripToInjectorQueue.front();
		DaDripToInjectorQueue.pop();
		return pk;
	};
	...
	int64_t getPlayerXuid() {
		auto readPtr = [](uintptr_t start, std::vector<size_t> offsets) {
			uintptr_t ptr = start;
		...
		uintptr_t ptrBase = (uintptr_t)GetModuleHandleA("Minecraft.Windows.exe");

		return *(int64_t*)readPtr(ptrBase + 0x3652658i64, {0x1E8, 0x8, 0x10, 0x0, 0x28, 0x70});
	};
	bool verifyXuid();
};
