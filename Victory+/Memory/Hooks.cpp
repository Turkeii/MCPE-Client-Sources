...
void Hooks::ClientInstanceScreenModel_sendChatMessage(void* _this, TextHolder* text) {
	...
	if (text->getTextLength() > 0) {
		...
		if (*message == cmdMgr->prefix) {
			...
			if (!helpedUser) {
				helpedUser = true;
				g_Data.getClientInstance()->getGuiData()->displayClientMessageF("%sYour KaeruVxint prefix is: \"%s%c%s\"", RED, YELLOW, cmdMgr->prefix, RED);
...

	{
		...
		if (!g_Data.isInjectorConnectionActive() && !hasSentWarning) {
			...
			if (elapsedFlot > 1.5f && !hasSentWarning) {
				hasSentWarning = true;
				//auto box = g_Data.addInfoBox("Welcome", "Injected EzCrystals!");
				//box->closeTimer = 5;
	...
	*/
	...
	}
	{
		...
		if (strcmp(screenName.c_str(), "start_screen") == 0) {
			// Draw BIG epic horion watermark
			{
				...
				DrawUtils::fillRectangle(rectPos, MC_Color(1.f, 1.f, 1.f, 1.f), 1.f);
				DrawUtils::drawRectangle(rectPos, MC_Color(1.f, 0.f, 1.f, 1.f), 2.f);
				DrawUtils::drawText(textPos, &text, MC_Color(0.f, 0.f, 0.f, 1.f), 8.f);
			}
...

			{
				...
				if (shouldRenderWatermark) {
					...
					static const float textHeight = (nameTextSize + versionTextSize * 0.7f /* We don't quite want the version string in its own line, just a bit below the name */) * DrawUtils::getFont(Fonts::LOGOFONT)->getLineHeight();
					constexpr float borderPadding = 1;
					constexpr float margin = 5;

					//std::string text = g_Data.getLocalPlayer()->getNameTag()->getText();

					//static std::string name = "Xalyfi";
					//static std::string name2 = "           Client";

					static std::string name = "Kaeru";
					static std::string name2 = "           Vxint"; 
					#ifdef _DEBUG
					static std::string version = "Debug";
#elif defined _BETA
					static std::string version = "Public";
#else
					static std::string version = "Beta Thanks testing";
					...
					vec4_t rect = vec4_t(
						0.f + margin - borderPadding + hudModule->posx,
						0.f + margin + hudModule->posy,
						0.f + margin + fullTextLength + borderPadding * 2,
						0.f + margin + textHeight);

					//DrawUtils::drawRectangle(rect, MC_Color(1.f, 1.f, 1.f), 1.f, 0.5f);
					//DrawUtils::fillRectangle(rect, MC_Color(12, 12, 12), 0.25f);
					DrawUtils::drawText(vec2_t(rect.x + borderPadding, rect.y), &name, MC_Color(1.f, 0.f, 0.6f), nameTextSize);
					DrawUtils::drawText(vec2_t(rect.x + borderPadding, rect.y), &name2, MC_Color(currColor), nameTextSize);
					DrawUtils::drawText(vec2_t(rect.x + 0, rect.w - 5.5), &version, MC_Color(1.f, 1.f, 1.f), versionTextSize);
				}
				...
				if (moduleMgr->isInitialized() && shouldRenderArrayList) {
					...
					struct IModuleContainer {
						...
						IModuleContainer(std::shared_ptr<IModule> mod) {
							...
							if (mode)
								...
								char text2[50];
								sprintf_s(text2, 50, "%s%s", moduleNameChr, hudModule->keybinds ? std::string(" " + std::string(Utils::getKeybindName(keybind)) + " " + (std::string(modNameChr))).c_str() : "");
								modName = text2;
							}
					...
					for (std::set<IModuleContainer>::iterator it = modContainerList.begin(); it != modContainerList.end(); ++it) {
						...
						float textWidth = it->textWidth + it->textWidth2 + 2.f;
						...
						underline = vec4_t(windowSize.x - (lastTextLenght + 1.18f + (textPadding * 1.18f)), leftRect.y, leftRect.x, leftRect.y + 0.2f);
						...
						DrawUtils::fillRectangle(rectPos, MC_Color(0.f, 0.f, 0.f), 0.2f);
						//DrawUtils::fillRectangle(leftRect2, MC_Color(1.f, 1.f, 1.f), 1.f);
						DrawUtils::fillRectangle(leftRect, MC_Color(Theme::rFloat, Theme::gFloat, Theme::bFloat), 0.7f);
						DrawUtils::fillRectangle(underline, MC_Color(Theme::rFloat, Theme::gFloat, Theme::bFloat), 0.7f);  //underline
						//DrawUtils::fillRectangle(rightbar, MC_Color(1.f, 1.f, 1.f), 1.f);  //bar

						underline = vec4_t(windowSize.x - (textWidth + 1.18f + (textPadding * 1.18f)), leftRect.w, windowSize.x, leftRect.w + 0.2f);
					...
					DrawUtils::fillRectangle(underline, MC_Color(Theme::rFloat, Theme::gFloat, Theme::bFloat), 1.f);  //underline
	...
	// Draw Info / Alert Boxes
	{
		...
		if (box) {
			...
			const float paddingHoriz = 40 * box->fadeVal - 2.f;
			const float paddingVert = 10 * box->fadeVal - 1.f;
			const float titleTextSize = box->fadeVal * 2 - 2.f;
			const float messageTextSize = box->fadeVal * 1 - 1.f;
			...
			DrawUtils::drawRectangle(rectPos, rcolors, box->fadeVal, 0.8f);
...
void Hooks::Actor_lerpMotion(C_Entity* _this, vec3_t motVec) {
	...
	static auto ReKBmod = moduleMgr->getModule<ReverseKnockBack>();
	...
	if (ReKBmod->isEnabled()) {
		static void* networkSender = nullptr;
		if (!networkSender)
			networkSender = reinterpret_cast<void*>(9 + FindSignature("48 8B CB FF ?? ?? ?? ?? 00 C6 47 ?? 01 48 8B 5C 24"));

		if (networkSender == _ReturnAddress()) {
			motVec = _this->velocity.lerp(motVec, -1, 1, -1);
		}
	}
...
void Hooks::LoopbackPacketSender_sendToServer(C_LoopbackPacketSender* a, C_Packet* packet) {
	...
	static auto Chatmod = moduleMgr->getModule<Chat>();

	if (Chatmod->isEnabled() && g_Data.isInGame()) {
		if (packet->isInstanceOf<C_TextPacket>()) {
			...
			std::string chat = Chatmod->getMessage();

			//std::string green = "§a";

			funi->message.setText(">" + msgtext + " | " + chat);
		}
	}
...
void Hooks::GameMode_startDestroyBlock(C_GameMode* _this, vec3_ti* a2, uint8_t face, void* a4, void* a5) {
	...
	if (PacketmineModule->isEnabled()) {
		if (PacketmineModule->dontBedrock && g_Data.getLocalPlayer()->region->getBlock(*a2)->toLegacy()->blockId != 7) {
			if (PacketmineModule->InstaDelay = Delay) {
	...
	if (instaBreakModule->isEnabled() && instaBreakModule->instant) {
		_this->destroyBlock(a2, face);
		return;
	}
	if (instaBreakModule->isEnabled() && instaBreakModule->packet) {
		if (g_Data.getLocalPlayer()->region->getBlock(*a2)->toLegacy()->blockId != 7) {
			_this->destroyBlock(a2, face);
			return;
		}
	}
	if (instaBreakModule->isEnabled() && instaBreakModule->haste) {
		Odelay++;
		if (g_Data.getLocalPlayer()->region->getBlock(*a2)->toLegacy()->blockId != 7) {
			if (Odelay == instaBreakModule->delay) {
				_this->destroyBlock(a2, face);
			}
			if (Odelay == instaBreakModule->delay) {
				Odelay = 0;
