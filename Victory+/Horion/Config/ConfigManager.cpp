...
void ConfigManager::loadConfig(std::string name, bool create) {
	...
	if (configExists || create) {
		...
		if (configExists) {
			...
			currentConfigObj["from"] = "KaeruVxint";
		}
		...
		if (g_Data.getLocalPlayer() != nullptr) {
			...
			g_Data.getGuiData()->displayClientMessageF("[%sKaeru Vxint%s] %sSuccessfully %s config %s%s%s!", DARK_PURPLE, WHITE, GREEN, !configExists ? "created" : "loaded", GRAY, name.c_str(), GREEN);
			if (!helpedUser && name != "default") {
				helpedUser = true;
				g_Data.getGuiData()->displayClientMessageF("[%sKaeru Vxint%s] %sEnter \"%s%cconfig load default%s\" to load your old config!", DARK_PURPLE, WHITE, YELLOW, WHITE, cmdMgr->prefix, YELLOW);
			}
		}
	} else {
		if (g_Data.getLocalPlayer() != nullptr) 
			g_Data.getGuiData()->displayClientMessageF("[%sKaeru Vxint%s] %sCould not load config %s%s%s!", DARK_PURPLE, WHITE, RED, GRAY, name.c_str(), RED);
	}
