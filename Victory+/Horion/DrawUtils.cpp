...
C_Font* DrawUtils::getFont(Fonts font) {
	...
	break;
	case Fonts::LOGOFONT:
	return g_Data.getClientInstance()->minecraftGame->getMojanglesFont();
		break;
	case Fonts::UNICOD:
...
void DrawUtils::drawLine1(const vec2_t& start, const vec2_t& end, float lineWidth) {
	float modX = start.y + end.y;
...
void DrawUtils::drawBoxv2(const vec3_t& lower, const vec3_t& upper, float lineWidth, bool outline, bool fill) {
	vec3_t diff;
...
void DrawUtils::drawBox2X(const vec3_t& lower, const vec3_t& upper, float lineWidth, bool outline, bool fill) {
	vec3_t diff;
	diff.x = upper.x - lower.x;
	diff.y = upper.y - lower.y + 1.f;
	diff.z = upper.z - lower.z;
	//diff.w = upper.w - lower.w;
...
void DrawUtils::drawBox(const vec3_t& lower, const vec3_t& upper, float lineWidth, bool outline, bool fill) {
