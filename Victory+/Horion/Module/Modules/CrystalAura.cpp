...
CrystalAura::CrystalAura() : IModule(VK_NUMPAD0, Category::COMBAT, "Auto Select Crystals") {
	...
	for (int x = (int)pos->x - eRange; x < pos->x + eRange; x++) {
		for (int z = (int)pos->z - eRange; z < pos->z + eRange; z++) {
			for (int y = (int)pos->y - eRange; y < pos->y + eRange; y++) {
				...
				vec3_ti sitaBlockPos = vec3_ti(x, y - 1, z + 1);
				C_Block* block = gm->player->region->getBlock(blockPos);
				C_Block* upperBlock = gm->player->region->getBlock(upperBlockPos);
				...
				if (block != nullptr) {
					...
					if ((blockId == 49 || blockId == 7) && upperBlockId == 0 && sitaBlockId == 0 && CanPlaceC(&blockPos)) {  //Check for awailable block
						if (!ValidPos) {
							ValidPos = true;
							bestPos = blockPos;
...
void CrystalAura::onTick(C_GameMode* gm) {
	...
	if (this->delay == 2) {
		if (autoplace && g_Data.getLocalPlayer()->getSelectedItemId() == 629 || g_Data.getLocalPlayer()->getSelectedItemId() == 615 || g_Data.getLocalPlayer()->getSelectedItemId() == 616) {  //endcrystal
			if (pEnhanced)
				for (auto& i : targetList)
					CPlace(gm, i->getPos());
			else {
