...
HudModule::HudModule() : IModule(0, Category::GUI, "Displays ArrayList") {
	...
	//registerBoolSetting("Show Mode", &this->Mode, this->Mode);
	registerBoolSetting("Show modes", &this->keybinds, this->keybinds);
	...
	registerFloatSetting("X", &this->posx, this->posx, 0.f, g_Data.getClientInstance()->getGuiData()->windowSize.x);
	registerFloatSetting("Y", &this->posy, this->posy, 0.f, g_Data.getClientInstance()->getGuiData()->windowSize.y);
...
void HudModule::onPostRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	{  // Coordinates
		if (!(g_Data.getLocalPlayer() == nullptr || !this->coordinates)) {
			...
			vec2_t textPos = vec2_t(rectPos.x + 1.5f, rectPos.y + 10.f);
