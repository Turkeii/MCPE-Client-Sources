...
void BPGRewrite::onPreRender(C_MinecraftUIRenderContext* ctx) {
	if (!g_Data.isInGame()) {
		...
		if (renderCrystal) {
			DrawUtils::setColor(0.f, 1.f, 1.f, 0.4f);
...
float BPGRewrite::computeExplosionDamage(vec3_t crystalPos, C_Entity* target, C_BlockSource* reg, int mode) {
	...
	if (dist <= 1.f) {
		...
		if (mode != dmg_vanilla) {
			exposure = 1;
			if (mode == dmg_java) {
				exposure = calculateBlockDensity(crystalPos, AABB(target->aabb.lower, vec3_t(target->aabb.upper.x, fminf(target->aabb.lower.y + 1.f, target->aabb.upper.y), target->aabb.upper.z)), reg);
			}
...
bool BPGRewrite::tryRaytrace(vec3_t vec31, vec3_t vec32, C_BlockSource* region) {
	...
	if (clip(region, result, vec3_t(vec31), vec32, false, true, 15, false, false)->hitType == 3) {
		return true;
	}
	return false;
}