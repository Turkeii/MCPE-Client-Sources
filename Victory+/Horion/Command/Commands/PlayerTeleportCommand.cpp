...
bool PlayerTeleportCommand::execute(std::vector<std::string>* args) {
	...
	if (pos.iszero()) {
		clientMessageF("[%sKaeru Vxint%s] %sCouldn't find player: %s!", DARK_PURPLE, WHITE, RED, nameOfPlayer.c_str());
		return true;
	}
	g_Data.getLocalPlayer()->setPos(pos);
	clientMessageF("[%sKaeru Vxint%s] %sTeleported to %s", DARK_PURPLE, WHITE, GREEN, playerName.c_str());
	return true;
}
