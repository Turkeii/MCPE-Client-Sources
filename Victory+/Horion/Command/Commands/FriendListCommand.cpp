...
bool FriendListCommand::execute(std::vector<std::string>* args) {
	...
	if (playerName.size() <= 1) {
		clientMessageF("[%sKaeru Vxint%s] %sCouldn't find player: %s!", DARK_PURPLE, WHITE, RED, searchedName.c_str());
		return true;
	}
	if (subcommand == "add") {
		FriendList::addPlayerToList(playerName);
		clientMessageF("[%sKaeru Vxint%s] %s%s is now your friend!", DARK_PURPLE, WHITE, GREEN, playerName.c_str());
		return true;

	} else if (subcommand == "remove") {
		if (FriendList::removePlayer(searchedName)) {
			clientMessageF("[%sKaeru Vxint%s] %s%s has been removed from your friendlist!", DARK_PURPLE, WHITE, GREEN, searchedName.c_str());
			return true;
		} else {
			clientMessageF("[%sKaeru Vxint%s] %s%s was not in your friendlist!", DARK_PURPLE, WHITE, GREEN, searchedName.c_str());
			return true;
		}
	}
	return true;
