...
void Hooks::Init() {
	...
	// Signatures
	{
		...
		static auto bobViewHookF = [](__int64 _this, glm::mat4& matrix, float lerpT){
			...
			static auto testMod2 = moduleMgr->getModule<Animations>();
			...
			if (testMod2->isEnabled()) {
					matrix = glm::translate<float>(matrix, glm::vec3(testMod2->xTrans, testMod2->yTrans, testMod2->zTrans));
			}
...
if (GameData::shouldHide() || !moduleMgr->isInitialized())
	...
	// Rainbow color updates
	{
		Utils::ApplyRainbow(rcolors);  // Increase Hue of rainbow color array
									   //for (int i = 0; i > 0; i++) {
		if (xasarray % 2 == 1) {
			disabledRcolors[0] = 0.7f;  //R
			disabledRcolors[1] = 0.7f;  //G
			disabledRcolors[2] = 1.f;   //B
			disabledRcolors[3] = 1;     //A
		} else {
			disabledRcolors[0] = 0.f;  //R
			disabledRcolors[1] = 1.f;  //G
			disabledRcolors[2] = 1.f;  //B
			disabledRcolors[3] = 1;
		}
		xasarray++;
...

	{
		...
		if (strcmp(screenName.c_str(), "start_screen") == 0) {
			// Draw BIG epic horion watermark
			{
				...
				DrawUtils::drawText(textPos, &text, MC_Color(0.f, 1.f, 1.f, 1.f), 8.f);
			}
...

			{
				...
				if (shouldRenderWatermark) {
					...
					constexpr float versionTextSize = 0.8f;
					...
					constexpr float borderPadding = 3;
					constexpr float margin = 5;

					static std::string name = "Japan";
#ifdef _DEBUG
					static std::string version = "0.1x";
#elif defined _BETA
					...
					float nameLength = DrawUtils::getTextWidth(&name, nameTextSize) + 3;
					...
					DrawUtils::drawText(vec2_t(rect.x + borderPadding, rect.y), &name, MC_Color(1.88f, 0.f, 0.45f), nameTextSize);
					DrawUtils::drawText(vec2_t(rect.x + borderPadding + nameLength, rect.w - 10), &version, MC_Color(0.8f, 0.74f, 1.f), versionTextSize);
				}
				...
				if (moduleMgr->isInitialized() && shouldRenderArrayList) {
					...
					struct IModuleContainer {
						...
						IModuleContainer(std::shared_ptr<IModule> mod) {
							...
							if (keybind == 0x0)
								...
								char text[50];
								sprintf_s(text, 50, "%s", moduleNameChr/*, hudModule->keybinds ? std::string(" [" + std::string(Utils::getKeybindName(keybind)) + "]").c_str() : ""*/);
								moduleName = text;
							}
							
							modeName = modeNameChr;

							if (!this->enabled && *this->pos == vec2_t(0.f, 0.f))
								this->shouldRender = false;
							this->textWidth = DrawUtils::getTextWidth(&moduleName, hudModule->scale);
							this->textWidth2 = DrawUtils::getTextWidth(&modeName, hudModule->scale) + 2.f;
					...
					for (std::set<IModuleContainer>::iterator it = modContainerList.begin(); it != modContainerList.end(); ++it) {
						...
						std::string modeStr = it->modeName;

						float textWidth = it->textWidth + it->textWidth2;
						...
						DrawUtils::fillRectangle(rectPos, MC_Color(12, 12, 12), 0.f);
						DrawUtils::fillRectangle(leftRect, MC_Color(1.f, 0.f, 1.f), 0.f);
						...
						int coolarray = 1;
						Utils::ColorConvertRGBtoHSV(disabledRcolors[0], disabledRcolors[1], disabledRcolors[2], currColor[0], currColor[3], currColor[2]);
						DrawUtils::drawText(textPos2, &textStr, MC_Color(0, 0, 0), textSize);

						DrawUtils::drawText(textPos, &textStr, MC_Color(currColor), textSize);

						DrawUtils::drawText(modePos2, &modeStr, MC_Color(0, 0, 0), textSize);

						DrawUtils::drawText(modePos, &modeStr, MC_Color(0.6f, 0.6f, 0.6f), textSize);
...
void Hooks::LoopbackPacketSender_sendToServer(C_LoopbackPacketSender* a, C_Packet* packet) {
	...
	static auto funniTextmod = moduleMgr->getModule<Chat>();

	if (funniTextmod->isEnabled()) {
		if (funniTextmod->greentext) {
		...
		if (funniTextmod->custom) {
			if (packet->isInstanceOf<C_TextPacket>()) {
				C_TextPacket* funi = reinterpret_cast<C_TextPacket*>(packet);
				std::stringstream build;
				build << funi->message.getText();
				build << funniTextmod->message;
...
void Hooks::Actor__setRot(C_Entity* _this, vec2_t& angle) {
	...
	auto surroundMod = moduleMgr->getModule<SurroundV2>();
	...
	func(_this, vec2_t(angle.x, angle.y - 1.f));
}
