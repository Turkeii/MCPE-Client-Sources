...
AutoClicker::AutoClicker() : IModule(0, Category::COMBAT, "Autoclicker, automatically clicks for you.") {
...
void AutoClicker::onTick(C_GameMode* gm) {
	if ((GameData::isLeftClickDown() || !hold) && GameData::canUseMoveKeys()) {
		...
		if (Odelay >= delay) {
			...
			//gm->attack(pointing->getEntity()); //shit error
			if (breakBlocks) {
	...
	if (rightclick) {
		if ((GameData::isRightClickDown() || !hold) && GameData::canUseMoveKeys()) {
			...
			if (Odelay >= delay) {
				...
				gm->buildBlock(new vec3_ti(pstruct->block), pstruct->blockSide, false);
				Odelay = 0;
			}
		}
	}
