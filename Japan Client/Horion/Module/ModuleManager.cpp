...
void ModuleManager::initModules() {
	logF("Initializing modules");
	{
		...
		moduleList.push_back(std::shared_ptr<IModule>(new AutoCrystalKari()));
	...
	moduleList.push_back(std::shared_ptr<IModule>(new TPAura()));
		moduleList.push_back(std::shared_ptr<IModule>(new SurroundV2()));
	...
	getModule<LongJump>()->setEnabled(false);
}
