...
static constexpr float backgroundAlpha = 0.1f;

static const MC_Color whiteColor = MC_Color(1.f, 1.f, 1.f, 0.5f);
...
void ClickGui::renderCategory(Category category) {
	...
	if (ourWindow->isExtended || ourWindow->isInAnimation) {
		...
		for (auto& mod : moduleList) {
			...
			// Text
			if (allowRender)
				DrawUtils::drawText(textPos, &textStr, mod->isEnabled() ? whiteColor : MC_Color(1.88f, 0.f, 0.45f), textSize);
						...
						for (auto setting : *settings) {
							...
							case ValueType::BOOL_T: {
								...
								// Text
								{
									...
									DrawUtils::drawText(textPos, &elTexto, MC_Color(1.88f, 0.f, 0.45f), textSize);
