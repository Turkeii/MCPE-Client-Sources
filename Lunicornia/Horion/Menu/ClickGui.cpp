...
static const MC_Color selectedModuleColor = MC_Color(0.27f, 0.33f, 0.43f, 0.6f);
static const MC_Color moduleColor = MC_Color(0.145f, 0.173f, 0.22f, 0.6f);
...
void ClickGui::renderCategory(Category category)
{
	...
	if (ourWindow->isExtended || ourWindow->isInAnimation) {
		...
		for (std::vector<IModule*>::iterator it = moduleList.begin(); it != moduleList.end(); ++it) 
		{
			...
			float IT = mod->hoverTime / 4.f + 1;
			// Text
			textPos.x += IT;

			if (allowRender)
				DrawUtils::drawText(textPos, &textStr, mod->isEnabled() ? new MC_Color(0.317f, 0.752f, 0.474f, 1.f) : new MC_Color(1.0f, 1.0f, 1.0f, 1.0f), textSize);

			textPos.x -= IT;
			// Settings
			{
				static vec2_t oldMousePos = vec2_t();
				...
				//#define difX (oldMousePos.x - currentXOffset)
				//#define difY (oldMousePos.y - currentYOffset)
				//	static vec2_t newMousePos = vec2_t(
				//		difX + mousePos.x,
				//		difY + mousePos.y
				//	);
					...
					{
						//Push;
				//		DrawUtils::translate(vec2_t(difX, difY));
						...
						for (auto it = settings->begin(); it != settings->end(); ++it) {
							...
							case ValueType::FLOAT_T:
							{
								...
								// Slider
								{
									...
									// Visuals & Logic
									{
										...
										rectPos.w += textHeight + (textPadding * 2) + 2;
										// Background
										...
										// Draw Int
										{
											...
											DrawUtils::drawText(mid, &text, (areWeFocused || setting->isDragging) ? new MC_Color(1.0f, 0.3f, 0.3f, 1.f) : new MC_Color(1.0f, 0.1f, 0.1f, 1.f), textSize);
										}
							...
							case ValueType::BYTE:
							{
								...
								DrawUtils::drawLine(vec2_t(currentXOffset + 2, currentYOffset - textHeight), vec2_t(currentXOffset + 2, currentYOffset), 0.5f);
							}
								break;
							default:
							{
								...
								std::string notHowSpanishWorks = alc;
								// Adjust window size if our text is too 
								windowSize->x = max(windowSize->x, DrawUtils::getTextWidth(&notHowSpanishWorks, textSize) + 5 /* because we add 5 to text padding*/);
								
								DrawUtils::drawText(textPos, &notHowSpanishWorks, new MC_Color(1.0f, 1.0f, 1.0f, 1.0f), textSize);
								currentYOffset += textHeight + (textPadding * 2);
							}
								break;
							}
	...
	// Draw Category Header
	{
		...
		// Draw component
		{
			...
			DrawUtils::fillRectangle(rectPos, MC_Color(0.145f, 0.173f, 0.22f, 1.f), 0.95f);
			DrawUtils::setColor(0.317f, 0.752f, 0.474f, 1.f);
			DrawUtils::drawLine(vec2_t(rectPos.x, rectPos.w), vec2_t(rectPos.z, rectPos.w), 0.5f);
			// Draw Dash
			GuiUtils::drawCrossLine(vec2_t(
				currentXOffset + windowSize->x + paddingRight - (crossSize / 2) - 1.f,
...
std::string* discText = new std::string("discord.gg/SXmbjED");
std::string* luniText = new std::string("Lunicornia Public");
...
void ClickGui::render()
{
	...
	// Fill Background
	{
		...
		), MC_Color(0.8f, 0.8f, 0.8f, 0.05f), 0.1f);

		//top bar
		DrawUtils::fillRectangle(vec4_t(0, 0,
			g_Data.getClientInstance()->getGuiData()->widthGame, 30
		), MC_Color(0.1686f, 0.196f, 0.239f, 0.6f), 1.f);

		//top line
		DrawUtils::fillRectangle(vec4_t(0, 30,
			g_Data.getClientInstance()->getGuiData()->widthGame, 32
		), MC_Color(0.3176f, 0.7529f, 0.4745f, 0.6f), 1.f);

		//watermark
		DrawUtils::drawText(vec2_t(4.f, 0.f), luniText, new MC_Color(0.3176f, 0.7529f, 0.4745f, 0.6f), 2.f);
		DrawUtils::drawText(vec2_t(4.f, 16.f), discText, new MC_Color(0.3176f, 0.7529f, 0.4745f, 0.6f), 1.f);
		...
		DrawUtils::fillRectangle(vec4_t(0,
			g_Data.getClientInstance()->getGuiData()->heightGame - 30,
			g_Data.getClientInstance()->getGuiData()->widthGame,
			g_Data.getClientInstance()->getGuiData()->heightGame - 28
		), MC_Color(0.3176f, 0.7529f, 0.4745f, 0.6f), 1.f);
	}
