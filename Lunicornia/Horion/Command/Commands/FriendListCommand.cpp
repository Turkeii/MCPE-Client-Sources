...
bool FriendListCommand::execute(std::vector<std::string>* args)
{
	...
	{
		clientMessageF("[%sLunicornia%s] %sCouldn't find player: %s!", GOLD, WHITE, RED, searchedName.c_str());
		return true;
	}
	if (subcommand == "add")
	{
		FriendList::addPlayerToList(playerName);
		clientMessageF("[%sLunicornia%s] %s%s is now your friend!", GOLD, WHITE, GREEN, playerName.c_str());
		return true;
	...
	else if (subcommand == "remove")
	{
		if (FriendList::removePlayer(searchedName)) {
			clientMessageF("[%sLunicornia%s] %s%s has been removed from your friendlist!", GOLD, WHITE, GREEN, searchedName.c_str());
			return true;
		}
		else {
			clientMessageF("[%sLunicornia%s] %s%s was not in your friendlist!", GOLD, WHITE, GREEN, searchedName.c_str());
			return true;
		}
	}
	return true;
