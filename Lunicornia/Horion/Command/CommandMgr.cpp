...
void CommandMgr::initCommands() {
	...
	//commandList.push_back(new GetPlayerCoords());
...
void CommandMgr::execute(char * message)
{
	if (message != nullptr) {
			...
			for (std::vector<std::string>::iterator it = aliases->begin(); it != aliases->end(); ++it) {
				if (*it == cmd) {
					try {
						...
						g_Data.getClientInstance()->getGuiData()->displayClientMessageF("%s%sInternal Exception has occured.", RED, BOLD);
					}
					goto done;
				}
			}
		...
		g_Data.getClientInstance()->getGuiData()->displayClientMessageF("[%sLunicornia%s] %sCommand '%s' could not be found!", GOLD, WHITE, RED, cmd.c_str());
