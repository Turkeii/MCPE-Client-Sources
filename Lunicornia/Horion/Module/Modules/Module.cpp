...
IModule::IModule(int key, Category c, const char* tooltip)
{
	...
	this->registerByteSetting(std::string("keybind"), &this->keybind, this->keybind, 0, 0xFF);
...
void IModule::registerByteSetting(std::string name, int* intPtr, int defaultValue, int minValue, int maxValue)
{
#ifdef DEBUG
	if (minValue > maxValue)
		__debugbreak(); // Minimum value is bigger than maximum value
	...
	SettingEntry* setting = new SettingEntry();
	setting->valueType = ValueType::BYTE;
	setting->value = reinterpret_cast<SettingValue*>(intPtr); // Actual Value
