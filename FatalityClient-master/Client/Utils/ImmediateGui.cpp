...
void ButtonInfo::calculateSize(const char* txt) {
	std::string str(txt);
	this->size = { DrawUtils::getTextWidth(&str), DrawUtils::getFont(Fonts::DEFAULT)->getLineHeight() };
}
...
void ButtonInfo::draw(vec2_t mousePos, const char* label) {
	...
	DrawUtils::drawText(textPos, &str, MC_Color(255, 255, 255), 1, 1);
	if (isInSelectableSurface(mousePos)) {  // Mouse hovering over us
		DrawUtils::drawRoundRectangle(surface, MC_Color(128, 128, 128, 80), false);
		this->canClickB = true;
	}
	else {
		DrawUtils::drawRoundRectangle(surface, MC_Color(0, 0, 0, 80), false);
