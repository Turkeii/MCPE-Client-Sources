...
const char* Animations::getModuleName() {
	...
	if (mode.getSelectedValue() == 1) name = string("Animations ") + string(GRAY) + string("Dev");
...
void Animations::onTick(C_GameMode* gm) {
	...
	if (shouldBlock) {
		...
		if (tap) Utils::nopBytes((unsigned char*)TapAddress, 4);
		else Utils::patchBytes((unsigned char*)((uintptr_t)TapAddress), (unsigned char*)"\xF3\x0F\x51\xF0", 4);
	}
