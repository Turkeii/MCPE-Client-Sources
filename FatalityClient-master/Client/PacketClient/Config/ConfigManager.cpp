...
void ConfigManager::loadConfig(std::string name, bool create) {
	...
	if (configExists || create) {
		...
		if (g_Data.getLocalPlayer() != nullptr) {
			g_Data.getGuiData()->displayClientMessageF("[F] %sSuccessfully %s config %s%s%s!", RED, !configExists ? "created" : "loaded", GRAY, name.c_str(), RED);
		}
	} else {
		//if (g_Data.getLocalPlayer() != nullptr) 
			//g_Data.getGuiData()->displayClientMessageF("[F] %sCould not load config %s%s%s!", RED, GRAY, name.c_str(), RED);
