...
bool FriendListCommand::execute(std::vector<std::string>* args) {
	...
	if (playerName.size() <= 1) {
		clientMessageF("[A] %sCouldn't find player: %s!", RED, searchedName.c_str());
		return true;
	}
	if (subcommand == "add") {
		FriendList::addPlayerToList(playerName);
		clientMessageF("[A] %s%s is now your friend!", RED, playerName.c_str());
		return true;

	} else if (subcommand == "remove") {
		if (FriendList::removePlayer(searchedName)) {
			clientMessageF("[A] %s%s has been removed from your friendlist!", GREEN, searchedName.c_str());
			return true;
		} else {
			clientMessageF("[A] %s%s was not in your friendlist!", GREEN, searchedName.c_str());
			return true;
		}
	}
	return true;
