...
bool SpammerCommand::execute(std::vector<std::string>* args) {
	...
	if (option == "text") {
		...
		clientMessageF("[F] %sSpammer message set to %s%s%s!", RED, GRAY, text.c_str(), RED);
		return true;
	} else if (option == "delay") {
		int delay = assertInt(args->at(2));
		if (delay < 1) {
			clientMessageF("[F] %sDelay needs to be 1 or more!", RED);
			return true;
		} else {
		...
		clientMessageF("[F] %sBypass set to %s%s%s!", RED, GRAY, state ? "true" : "false", RED);
		return true;
